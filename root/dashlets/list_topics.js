...
(function(params){
    ...
    var names = {
        name: 'ID', title: 'Title', assignee: 'Assignee', name_status: 'Status', created_by: 'Created By', created_on: 'Created On', modified_by: 'Modified By', modified_on: 'Modified On' };
    var columns = [{
        name:'name'}
                   , {
                       name:'title'}
                   ...
                   , {
                       name:'created_on'}
                  ];//, {name:'modified_by'}, {name:'modified_on'}];
...
function insert_number_html (topic,col,decimal_zone,totals,html){
    ...
    if ( col.total && topic[col.name] && !jQuery.isArray( topic[col.name] ) && ( parseFloat( topic[col.name] ) - parseFloat( topic[col.name] ) + 1) >= 0) {
        if ( totals[col.name] ) {
            ...
            totals[col.name] = {
                precision: precision, sum: parseFloat(topic[col.name]).toFixed(precision), count: 1, min: parseFloat(topic[col.name]).toFixed(precision), max: parseFloat(topic[col.name]).toFixed(precision) };
        }
    }
