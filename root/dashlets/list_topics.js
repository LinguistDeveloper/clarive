...
(function(params){ 
    ...
    Cla.ajax_json('/dashboard/list_topics', { assigned_to: assigned_to, condition: condition, not_in_status: not_in_status, categories: categories, statuses: statuses }, function(res){
        ...
        html = html + '<table class="table display nowrap stripe order-column compact" style="font-size: 85%;width: 100%" id="<% $iid %>"><thead><tr>';
        ...
        Ext.each( res.data, function(topic) {
          ...
          Ext.each( columns, function(col) {
            ...
            if ( col.name == 'name' ) {
              ...
              html = html + '<span title="'+title+'">'+title.substring(0,30)+'</span>'
