...
(function(){
	...
	var render_title = function(value,metadata,rec,rowIndex,colIndex,store) {
		...
		if(rec.data.labels){
			for(i=0;i<rec.data.labels.length;i++){
				tag_color_html = tag_color_html + "<span class='badge' style='float:left;padding:2px 8px 2px 8px;color:#" + returnOpposite(rec.data.labels[i].color) + ";background-color:#" + rec.data.labels[i].color + "'>" + rec.data.labels[i].name + "</span>";
			}
		}
	...
	var render_project = function(value,metadata,rec,rowIndex,colIndex,store){
		if(rec.data.projects){
			for(i=0;i<rec.data.projects.length;i++){
				...
				tag_project_html = tag_project_html + "<div class='alert' style='float:left'><button class='close' data-dismiss='alert'>×</button>" + rec.data.projects[i].project + "</div>";
			}
		}
