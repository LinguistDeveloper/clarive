(function(params) {
    ...
    store_actions.on('load', function(ds, records, o) {
        Ext.each(records, function(record) {
            actions.push({
                boxLabel: _(record.data.action),
                name: 'action',
                inputValue: record.data.action,
    ...
    var add_edit = function(rec) {
        ...
        var cb_events = new Ext.ux.form.SuperBoxSelect({
            ...
            store: store_events,
            singleMode: true,
            tpl: '<tpl for="."><div class="x-combo-list-item">' + '<span id="boot" style="background: transparent"><strong>[{kind}] {description}</strong><p>{key}</p></span></div></tpl>'
        });
                ...
                if (res.success) {
                    ...
                    if (scopes) {
                        ...
                        for (var i = 0; i < scopes.length; i++) {
                            switch (scopes[i]) {
                                case 'project':
                                    var store_projects = new Baseliner.store.UserProjects({
                                        id: 'id',
                                        baseParams: {
                                            include_root: true
                                        }
                                ...
                                case 'bl':
                                    ...
                                    store_baseline.on('load', function(ds, records, o) {
                                        if (rec && rec.data) {
                                            ...
                                            if (rec.data.data.scopes.bl && rec.data.data.scopes.bl['*']) {
                                                chk_baseline.setValue(true);
                                            } else {
                                                for (var propertyName in rec.data.data.scopes.bl) {
                                ...
                                case 'status':
                                    var store_status = new Baseliner.JsonStore({
                                        root: 'data',
                                        remoteSort: true,
                                        totalProperty: "totalCount",
                                        url: '/notification/list_status_end',
                                        fields: ['name']
                                    });

                                    store_status.on('load', function(ds, records, o) {

                                        if (rec && rec.data) {
                                            var ids_status = [];
                                            if (rec.data.data.scopes.status && rec.data.data.scopes.status['*']) {

                                                chk_status.setValue(true);
                                            } else {
                                                for (var propertyName in rec.data.data.scopes.status) {
                                                    ids_status.push(propertyName);
                                                    cb_status.setValue(ids_status);
                                                }
                                            }
                                        }
                                    });

                                    var cb_status = new Ext.ux.form.SuperBoxSelect({
                                        mode: 'local',
                                        triggerAction: 'all',
                                        forceSelection: true,
                                        fieldLabel: _('Status'),
                                    ...
                                    cb_status.on('additem', function(combo, value, record) {
                                        names_status[value] = record.data.name;

                                    });

                                    var chk_status = new Ext.form.Checkbox({
                                        name: 'status',
                                        boxLabel: _('All'),
                                        listeners: {
                                            check: function(obj, checked) {
                    ...
                    Ext.each(carriers, function(carrier) {
                        ...
                        if (ids.length == 0) {
                            ...
                            d = {
                                id: id,
                                recipients: carrier,
                                type: cb_type_recipient.getValue()
                            };
                        ...
                        Ext.each(ids, function(id_recipient) {
                            ...
                            d = {
                                ...
                                recipients: carrier,
                                type: cb_type_recipient.getValue(),
                                items_id: id_recipient,
                                items_name: is_text ? id_recipient : names_recipients[id_recipient]
                            };
                            r = new store_recipients.recordType(d, id);
                            store_recipients.add(r);
        ...
        var save_notification = function() {
            ...
            if (form.isValid()) {
                ...
                if (form.findField('bl') && form.findField('bl').getValue() != '') {
                    var bl = form.findField('bl').getValue().split(',');
                    var bl_names = {};
                    Ext.each(bl, function(bl) {
                        bl_names[bl] = names_baselines[bl];
                    });

                    params.bl_names = Ext.util.JSON.encode(bl_names);
                };
                ...
                for (type in rec.data.data.recipients[carrier]) {
                    ...
                    for (var name in rec.data.data.recipients[carrier][type]) {
                        ...
                        d = {
                            ...
                            type: type,
                            items_id: name,
                            items_name: rec.data.data.recipients[carrier][type][name]
                        };
                        r = new store_recipients.recordType(d, id);
                        store_recipients.add(r);
                    }
    ...
    var btn_start = new Baseliner.Grid.Buttons.Start({
        handler: function() {
            ...
            Baseliner.ajaxEval('/notification/change_active', {
                    ids_notification: notifications_checked,
                    action: 'active'
                },
                function(resp) {
    ...
    var btn_delete = new Baseliner.Grid.Buttons.Delete({
        handler: function() {
            ...
            Ext.Msg.confirm(_('Confirmation'), _('Are you sure you want to delete the notifications selected?'),
                function(btn) {
                    if (btn == 'yes') {
                        Baseliner.ajaxEval('/notification/remove_notifications', {
                                ids_notification: notifications_checked
                            },
                            function(response) {
                                if (response.success) {
    ...
    var notify_export = function(sel) {
        ...
        Baseliner.ajaxEval('/notification/export', {
            id_notify: ids
        }, function(res) {
            if (!res.success) {
    ...
    var notify_import = function() {
        ...
        var results = new Baseliner.MonoTextArea({
            flex: 1
        });
        var win = new Baseliner.Window({
            title: _('Import'),
    ...
    var show_recipients = function(value, metadata, rec, rowIndex, colIndex, store) {
        ...
        var carrierKeys = Object.keys(value.recipients).sort();
        var carrierLength = carrierKeys.length;
        for (var i = 0; i < carrierLength; i++) {
            var carrier = carrierKeys[i];
            ret += '<th>' + carrier + '</th>';
            var typeKeys = Object.keys(value.recipients[carrier]).sort();
            var typeLength = typeKeys.length;
            for (var j = 0; j < typeLength; j++) {
                var type = typeKeys[j];
                ret += '<tr>';
                ret += '<td style="font-weight: bold;padding: 3px 3px 3px 3px;">' + _(type) + '</td>';
                var nameKeys = Object.keys(value.recipients[carrier][type]).sort();
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        columns: [
            check_notifications_sm, {
                ...
                width: 200,
                dataIndex: 'data',
                renderer: show_recipients
            }, {
                header: _('Scopes'),
                width: 200,
                dataIndex: 'data',
                renderer: show_scopes
            }, {
                header: _('Action'),
                width: 50,
                dataIndex: 'action',
                renderer: show_action
            },
