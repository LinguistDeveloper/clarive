...
% } else {
    var fpanel = new Ext.FormPanel({
        ...
        buttons: [
%if($c->stash->{create}){
            {  icon:IC('create-slot-active'), text: _('Create Active Slot'), handler: function(){ modify_window('A') } }
            ,{ icon: IC('create-slot-inactive'), text: _('Create Inactive Slot'), handler: function(){ modify_window('AD') } }
%} else {
...
%   if( $activa ) {
            ,{ icon: IC('stop'), text: _('Disable (No Job)'), handler: function(){  modify_window('C0')   } }
%   } else {
            ,{ icon: IC('start'), text: _('Activate Slot'), handler: function(){  modify_window('C1')   } }
%     }
% }
