...
(function(){
    ...
    var render_icon = function(value,metadata,rec,rowIndex,colIndex,store) {
        ...
        if( active!=undefined && !active ) {
            value = '/static/images/icons/inactive-red.svg';
        }
    ...
    store_types.load();
    var button_add = new Ext.Button({ text: _('Add'), menu: menu_add , icon: '/static/images/icons/new-orange.svg', cls: 'x-btn-text-icon' });
    ...
    // YAML editor
    var button_raw = Baseliner.button(_('YAML'), '/static/images/icons/yaml-purple.svg', function(){
        var sel = sm.getSelected();
        if( sel == undefined ) return ;
    ...
    var button_by_type = new Ext.Button({
       text: _('Type'),
       icon: '/static/images/icons/catalog-blue.svg',
       cls: 'x-btn-text-icon',
       pressed: true,
       enableToggle: true,
       toggleGroup: 'grouping',
