(function(d) {
    ...
    Baseliner.confirm( _('Are you sure you want to deploy/rollback %1 to baseline %2?', String.format("<b>{0}</b>", name), String.format("<b>{0}</b>",_(to_state_name)) ), function() { 
        Baseliner.ajaxEval( '/topic/newjob', { mids:[mid], bl: bl_to, job_type: job_type, status_to: status_to, status_from: status_from, id_status_from: id_status_from }, function(res) {
            if( res.success ) {
