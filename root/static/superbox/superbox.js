...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect, Ext.form.ComboBox, {
    ...
    setMode : function(singleMode) {
        //如果从多选变成单选，则清空选项。
        if (singleMode != this.singleMode && singleMode && this.items.length > 1)
            this.removeAllItems();
                ...
                'remove': function(item) {
                    ...
                    if (this.removeValuesFromStore) {
                        if (this.usedRecords.containsKey(item.value)) {
                            ...
                            //TODO: 在mode == 'remote' 的时候， sortStore方法会导致请求服务器。应该彻查，不必要sort的就不要sort.
                            if (this.mode != 'remote')
                                this.sortStore();
                            if (this.view) {
                                this.view.render();
    ...
    onSelect : function(record, index) {
        if (this.fireEvent('beforeselect', this, record, index) !== false) {
            ...
            if (this.store.getCount() === 0 || !this.multiSelectMode || this.singleMode) {
                this.collapse();
            } else {
                this.restrictHeight();
            }
