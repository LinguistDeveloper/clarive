(function(params) {
        ...
        checkHandler: function() {
            if (!checked_event_details) {
                ...
                try {
                    ...
                    container.destroy();
                    Baseliner.ajaxEval('/comp/topic/topic_lib.js', {
                        self: pn_diagram.self
                    }, function() {
                        pn_diagram.self.show_timeline();
                    });
                }
            }
        }
    ...
    //Zoom +
    var btn_increaseZoom = new Ext.Button({
        text: _('Zoom +'),
        handler: function() {
    ...
    //Zoom -
    var btn_decreaseZoom = new Ext.Button({
        text: _('Zoom -'),
        handler: function() {
        ...
        function showMessage(e, obj) {
            ...
            if (obj.Eh.data_username !== undefined && obj.Eh.data_when !== undefined) {
                win = new Ext.Window({
                    ...
                    modal: false,
                    closeAction: 'hide',
                    items: [new Baseliner.MonoTextArea({
            ...
            go_api(go.Node, {
                ...
                go_api(go.Panel, "Auto",
                    go_api(go.Shape, "Rectangle", {
                        ...
                        new go.Binding("height", "duration", function(h) {
                            if (h > 0) {
                                h = computeActivityHeight(h);
                            } else {
                                h = computeActivityHeight2(h);
                            }
                            return h;
        ...
        diagram.linkTemplate =
            go_api(MessageLink, {
                ...
                go_api(go.Picture, {
                        ...
                        height: 28,
                        segmentIndex: 0,
                        segmentOffset: new go.Point(NaN, 20)
                    },
                ...
                go_api(go.TextBlock, {
                    ...
                    new go.Binding("visible", "isSelected", function(b) {
                        return b ? true : false;
                    }).ofObject(),
                    new go.Binding("text", "selected_text"),
                    new go.Binding("stroke", "isSelected", function(b) {
                        return b ? "#FFFFFF" : "transparent";
                    }).ofObject(),
                    new go.Binding("background", "isSelected", function(b) {
                        return b ? "#1E90FF" : "transparent";
                    }).ofObject())
            );

        // create the graph by reading the JSON data saved in "mySavedModel" textarea element
        Baseliner.ajaxEval('/topic/timeline_list_status_changes', {
            mid: mid
        }, function(res) {
            ...
            for (var i = 0; i < res.data.length; i++) {
                ...
                if ((res.data[i].data_type == "create" || res.data[i].data_type == "change_status") && res.data[i].status) {
                    ...
                    object_link.push({
                        "from": res.data[i].old_status,
                        "to": res.data[i].status,
                        "source": source,
                        "text": _('Username') + " : " + res.data[i].username + "\n" + _('Date') + ": " + Cla.user_date(res.data[i].when),
                        "time": start[j],
                        selected_text: "" + "\n" + _('Username') + ": " + res.data[i].username + "\n" + _('from') + ": " + res.data[i].old_status + "\n " + _('to') + ": " + res.data[i].status + "\n" + _('Date') + ": " + Cla.user_date(res.data[i].when) + "\n"
                    });
    ...
    function status_into_nodes(res) {
        ...
        for (i = 0; i < res.data.length; i++) {
            ...
            if (res.data[i].data_type == "create" || res.data[i].data_type == "change_status") {
                ...
                temp_status.push({
                    "old_status": res.data[i].old_status,
                    "status": res.data[i].status,
                    "when": res.data[i].when
                });
        ...
        for (i = 0; i < res.data.length; i++) {
            if (res.data[i].data_type == "create" || res.data[i].data_type == "change_status") {
                if (object_node.length === 0) {
                    object_node.push({
                        "group": "title",
                        "key": res.data[i].old_status,
                        "text": res.data[i].old_status,
                        "isGroup": true,
                        "loc": "0 0",
                        "duration": "change"
                    });
        ...
        for (i = 0; i < res.data.length; i++) {
            ...
            if (res.data[i].data_type == "create" || res.data[i].data_type == "change_status") {
                ...
                if (j === 0) {
                    ...
                    object_node.push({
                        "group": res.data[i].status,
                        "start": start[j],
                        "duration": duration[j],
                        "key": -(i + res.data.length),
                        "when": res.data[i].when,
                        "fill": "white",
                        "stroke": "black"
                    });
            ...
            if (res.data[i].data_type == "change_status") {
                ...
                while (j < res.data.length && res.data[i].when >= res.data[j].when) {
                    if (res.data[j].data_type == "topic_modify") {
                        ...
                        object_node.push({
                            "group": res.data[i].old_status,
                            "start": start_node + count_nodes + aux_count_nodes,
                            "duration": 0,
                            "key": -(j + i + res.data.length),
                            "text": "",
                            "fill": getLuxColor(change_stroke, 0.3),
                            "stroke": change_stroke,
                            "data_username": res.data[j].username,
                            "data_when": res.data[j].when,
                            "data_type": res.data[j].data_type,
                            "data_text": res.data[j].text,
                            "data_field": res.data[j].field,
                            "data_old_value": res.data[j].old_value,
                            "data_new_value": res.data[j].new_value
                        });
                        ...
                        object_node.push({
                            ...
                            "data_when": res.data[j].when,
                            "data_type": res.data[j].data_type,
                            "data_filename": res.data[j].filename,
                            "data_text": res.data[j].text
                        });
                        count_nodes = count_nodes + aux_count_nodes + 0.3;
                    } else {
                ...
                if (res.data[i].data_type == "topic_modify") {
                    change_stroke = color_green;
                    object_node.push({
                        "group": last_node.group,
                        "start": last_node.start + count_nodes_2,
                        "duration": 0,
                        "key": -(i + i + res.data.length),
                        "text": "",
                        "fill": getLuxColor(change_stroke, 0.3),
                        "stroke": change_stroke,
                        "data_username": res.data[i].username,
                        "data_when": res.data[i].when,
                        "data_type": res.data[i].data_type,
                        "data_text": res.data[i].text,
                        "data_field": res.data[i].field,
                        "data_old_value": res.data[i].old_value,
                        "data_new_value": res.data[i].new_value
                    });
                    ...
                    object_node.push({
                        ...
                        "data_when": res.data[i].when,
                        "data_type": res.data[i].data_type,
                        "data_filename": res.data[i].filename,
                        "data_text": res.data[i].text
                    });
                } else {
                    change_stroke = "black";
