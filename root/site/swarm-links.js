Cla.Swarm = Ext.extend( Ext.Panel, {
    ...
    add_single_node : function(a,d){
        ...
        self.node = self.node.data(self.force.nodes(), function(d) { return d.id;});
        self.node.enter().append("circle").attr("class", function(d) { return "node " + d.id; }).attr("r", 6).attr('fill',d.color||'blue').on("zoom", function(){self.rescale()});
        self.node.exit().remove();
    ...
    add_inicial : function(array){
        ...
        var self = this;
        var a = self.nodes[0];
        var d = {id: "#d"+Math.random(), t: "iniciales", ev: "iniciales", who: "iniciales", node: "iniciales", parent: array, color: "blue", posicionx: 0, posiciony: 0};           //var d = {id: "#d"+Math.random(), node: array};
