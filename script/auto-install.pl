...
sub restart {
    say "**** Restarting auto-installer... ($0)" ;
    exec "perl $0";
}
...
sub process_error {
    my $err = shift;
    my $mod;
    if( ($mod) = $err =~ /Can't locate (\S+)/ ) {
        if( length $mod && $mod ne 'object' ) {
            $mod =~ s{/}{::}g;
            $mod =~ s{\.pm$}{}g;
            install $mod;
            restart;
    ...
    elsif( ($mod) = $err =~ /Base class package "(\S+?)"/ ) {
        install $mod;
        restart;
    }
    elsif( ($mod) = $err =~ /Unable to load store class, (\S+)/ ) {
        install $mod;
        restart;
    }
    elsif( $err =~ /Couldn't find a JSON package/ ) {
        install 'JSON::XS';
        install 'JSON';
        restart;
    }
        ...
        say "No errors trapped.";
    }
    return 0;  # zero means not processed
}

package main ; {
    say "Baseliner auto-install facility (with included App::cpanminus)";
    %opts = _get_options();
    ...
    say "*******\nTrying to start the bali command...";
    my $ret = `perl script/bali.pl --env t 2>&1`;
    if( $? ) {
        say $ret;
        process_error( $ret );
    }

    say "*******\nTrying to start the bali server...";
    my $ret = `perl script/bali.pl --env t server 2>&1`;
    if( $? ) {
        say $ret;
        process_error( $ret );
    }
    say "\nDone. Now try running Baseliner for real with 'bin/bali' or 'bin/bali server -d'";
}
