...
DROP TABLE bali_sqa CASCADE CONSTRAINTS;

DROP SEQUENCE sq_bali_sqa_id;

CREATE SEQUENCE sq_bali_sqa_id;
...
ALTER TABLE bali_sqa ADD CONSTRAINT bali_sqa_id_prj_fk FOREIGN KEY (id_prj) REFERENCES bali_project (id);
...
CREATE OR REPLACE TRIGGER ai_bali_sqa_id
BEFORE INSERT ON bali_sqa
FOR EACH ROW WHEN (
 new.id IS NULL OR new.id = 0
)
BEGIN
 SELECT sq_bali_sqa_id.nextval
 INTO :new.id
 FROM dual;
END;
/
...
/
    ...
    DROP SEQUENCE BALI_SQA_SEQ;

    CREATE SEQUENCE BALI_SQA_SEQ
      START WITH 1
      MAXVALUE 999999999999999999999999999
      MINVALUE 1
      NOCYCLE
    ...
    CREATE OR REPLACE TRIGGER BALI_SQA_TRG
        BEFORE INSERT ON BALI_SQA
        REFERENCING NEW AS NEW OLD AS OLD
        FOR EACH ROW
        BEGIN
           SELECT BALI_SQA_SEQ.NEXTVAL INTO :NEW.ID FROM dual;
        END;
