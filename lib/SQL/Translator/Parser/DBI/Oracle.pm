...
sub parse {
    ...
    while(my $table_info = $sth->fetchrow_hashref('NAME_uc')) {
        ...
        while(my $column = $sth->fetchrow_hashref('NAME_uc')) {
            # return an scalar if its a literal
            #   a ref if its a bare value
            my $default_value = $column->{COLUMN_DEF} =~ /^'(.*)'\s*$/
                ? $1
                : $column->{COLUMN_DEF}; #uc $column->{COLUMN_DEF} eq 'NULL'
                    #? $column->{COLUMN_DEF} : \( $column->{COLUMN_DEF} );
            if( $column->{TYPE_NAME} =~ /integer|number|numeric/i ) {
                $default_value =~ s{\s+}{}g;
            }
            elsif( $column->{TYPE_NAME} =~ /date/i ) {
                $default_value =~ s{\s+$}{}g;
                $default_value = \"sysdate" if $default_value =~ /sysdate/i;
            }
            $ENV{DEBUG} && warn "\t\tTAB=$table_info->{TABLE_NAME}, DEF=[$default_value], COL=[$column->{COLUMN_DEF}]" if defined $column->{COLUMN_DEF};
            my $f = $table->add_field(
                name          => lc $column->{COLUMN_NAME},
                default_value => $default_value,
                data_type     => lc $column->{TYPE_NAME},
        ...
        while(my $primary_key = $sth->fetchrow_hashref('NAME_uc')) {
            my $f = $table->get_field( lc $primary_key->{COLUMN_NAME} );
            $f->is_primary_key(1);
        }
