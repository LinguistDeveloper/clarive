...
#
    ...
    sub create_index
    {
        ...
        $index_table_name = $index_table_name; #_generator()->quote($index_table_name);
        ...
        if ( $c->type eq PRIMARY_KEY ) {
            ...
            return sprintf 'UNIQUE %s(%s)',
              ((defined $c->name && $c->name)
                ? join ('',
                    $qf,
                    truncate_id_uniquely( $c->name, $options->{max_id_length} || $max_id_length ),
                    $qf,
                    ' '
                  )
                : ''
            ...
            my $table = $c->table;
            my $c_name = truncate_id_uniquely( $c->name, $options->{max_id_length} || $max_id_length );
