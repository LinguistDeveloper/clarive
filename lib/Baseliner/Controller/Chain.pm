...
register 'menu.admin.chain' => {
  ...
  action   => 'action.bde.view_distribution_chains'
};
...
sub data_grid_filter : Local {
  ...
  my @data = table_data('Baseliner::BaliChainedRule', $where);
...
sub view_dsl : Local {
  ...
  my $rs = 
    Baseliner->model('Baseliner::BaliChainedRule')->search({id => $id},
                      {select => ['dsl', 'dsl_code'], as => ['dsl', 'code']});
  rs_hashref($rs);
  my $row  = $rs->next;
  my $dsl  = $row->{dsl};
  my $code = $row->{code};
...
sub filter_dsl : Local {
  my ($self, $code) = @_;
  my @forbidden_words = ('die', '_throw', 'return',);
