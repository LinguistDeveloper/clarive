...
sub run_schedule : Local {
    ...
    try {
        if ($id) {
            BaselinerX::Model::SchedulerModel->schedule_task(
                taskid => $id,
                when   => 'now'
            );
        }
...
sub toggle_activation : Local {
    ...
    try {
        if ($id) {
            $new_status = BaselinerX::Model::SchedulerModel->toggle_activation(
                taskid => $id,
                status => $status
            );
        }
...
sub update_conf : Local {
    ...
    if ( ref $service ) {
        ...
        $service->update;
        $c->stash->{json} =
          { success => \1, msg => _loc("Configuration changed") };
    }
    else {
        $c->stash->{json} =
          { success => \0, msg => _loc('Error changing the configuration') };
    }
    $c->forward('View::JSON');
}
