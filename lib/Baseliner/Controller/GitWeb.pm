package Baseliner::Controller::GitWeb;
use Baseliner::Plug;
use Baseliner::Utils;
use Baseliner::Sugar;
use Try::Tiny;
...
sub gitweb : Path('/gitweb.cgi') {
    ...
    $self->cgi_to_response(
        ...
        sub {
            ...
            #$ENV{'GITWEB_CONFIG_SYSTEM'} || "/etc/gitweb.conf";
            #gitweb_config();
            my $config = config_get 'config.git';
            $self->{git_path} = $config->{path};
            $self->{projectroot} = ''; #$config->{home};   # TODO should be a list for the current user
            $self->{projects_list} = [
            ];
            $self->gitweb_cgi($c);
            #my $q = CGI->new;
