...
=head1 USAGE
 ...
 my $data = Baseliner::CSV->load(file       => $filename   [,%options]);
 my $data = Baseliner::CSV->load(filehandle => $filehandle [,%options]);
 my $data = Baseliner::CSV->load(string     => $string     [,%options]);

 # create a string of CSV from an array of hashes
 my $csv  = Baseliner::CSV->create( input => \@array_of_hashes [,%options]);

=head1 METHODS

=head2 new

 my $slurp = Baseliner::CSV->new();

Instantiate an object.

=head2 load

  my $data = Baseliner::CSV->load(file => $filename);
...
to determine the fields and order in which they appear in the CSV. For example:
 ...
 my $csv = Baseliner::CSV->create( input => \@array_of_hashes, field_order => ['one','three','two'] );
