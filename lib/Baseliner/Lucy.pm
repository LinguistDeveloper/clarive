package Baseliner::Lucy;
# stolen from LucyX::Simple
...
has highlighter => ( is => 'ro', isa => 'Str', );
has excerpt_length => ( is => 'rw', isa => 'Num', default=>300 );
    ...
    while ( my $hit = $hits->next ) {
        ...
        if( my $highlighter_class = $self->highlighter ) {
            my $highlighter = $highlighter_class->new( 
                ...
                query    => $query,
                field    => 'text',
                excerpt_length => $self->excerpt_length // 300,
            );
...
{
    package Baseliner::Lucy::Result::Hash;
    use strict;
    use warnings;
