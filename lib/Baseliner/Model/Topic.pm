...
sub get_system_fields {
    ...
    my @system_fields = (
        ...
        {
            id_field => 'created_by',
            params   => { name_field => 'Created By', bd_field => 'created_by', origin => 'default', meta_type => 'user' }
        },
        {
            id_field => 'created_on',
        ...
        },
        {
            ...
            params   => { name_field => 'Modified By', bd_field => 'modified_by', origin => 'default', meta_type => 'user' }
        },
        {
        ...
        if( $mt eq 'calendar' ) {
            ...
            for my $cal ( _array($arr) ) {
                ...
                if( length $cal->{slotname} ) {
                    my $slot = $cal->{slotname} ne '*' ? Util->_name_to_id( $cal->{slotname} ) : $cal->{slotname};
                    $doc->{$field}{$slot} = $cal;
                } else {
...
sub set_topics {
    ...
    if(@old_topics){
        ...
        # Take field 'to_mid' to look for references to remove in topic fields.
        my ($child_mid)  = _array (map { $_ ->{to_mid} } 
