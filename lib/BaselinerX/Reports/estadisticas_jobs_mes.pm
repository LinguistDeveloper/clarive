package BaselinerX::Reports::estadisticas_jobs_mes;
use Baseliner::Plug;
use Baseliner::Utils;
use Baseliner::Sugar;
use utf8;
...
register 'report.clarive.job_statistics_mes' => {
    ...
    security_handler => sub{
        my ($self,$username) =@_;
        my $config = config_get 'config.reports.estadisticas_jobs_mes';
    ...
    meta_handler => sub {
        ...
        return {
            fields => {
                ids => [
                    'mes', 'fallidos','pct_fallidos', 'cancelados','pct_cancelados', 'correctos', 'pct_correctos', 'total_ejecuciones', 'avg_ejecuciones',
                    'total'
                ],
                columns => [
                    {id => 'mes',  text => 'Mes'},
