...
sub best_match_on_viagra {
    ...
    unless( @ret || ($ns eq '/' && $bl eq '*') ) {
        if( $bl ne '*' ) {
            for( @values ) {
                return $_->{value}
                    if( ($_->{bl} eq '*') && ($_->{ns} eq $ns) );
            }
        }
        if( $ns ne '/' ) {
            try {
                my $ns_obj = Baseliner->model('Namespaces')->get( $ns );
                ...
                foreach my $parent ( @parents ) {
                    @ret = best_match_on_viagra( $parent, $bl, @values );
                    return @ret if @ret;
                }
            } catch {
                 die shift;
...
# turns config.aa.bb => ( config.aa, bb )
sub split_key {
    my ($self, $key ) = @_;
...
# returns the metadata of a config key
