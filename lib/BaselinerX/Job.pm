...
register 'config.job' => {
    metadata=> [
        ...
        { id=>'runner', label => 'Registry Entry to run', type=>'text', default=>sub { Clarive->config->{job_runner} || 'service.job.runner.rule' } },
...
register 'action.job.delete' => { name=>'Delete Job' };
