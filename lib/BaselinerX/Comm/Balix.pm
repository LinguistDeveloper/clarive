...
sub new {
    ...
    eval {
        ...
        local $SIG{ALRM} = sub {
            die "Timeout. Se ha sobrapasado el tiempo fijado ($p{timeout} seg) para la conexi髇 por agente a $p{host}:$p{port}.\n";
        };
        if ( $p{timeout} ne -1 ) {
            alarm $p{timeout};
        }
    ...
    if ($@) {
        croak "Error de conexi髇 por agente: $@";
    }
    return $balix;
}
...
sub open {
    ...
    my $socket = _open_socket( $rm, $rport )
      or croak
      "Balix: Error: no he podido conectarme a la m谩quina $rm:$rport : $@\n";
...
#or croak "Balix: Error: no he podido conectarme a la m谩quina $rm:$rport : $@\n";
#select($socket); $| = 1; select(stdout);
    if ($socket) {
        $socket->autoflush(1);
        binmode $socket;
...
Opciones:
 ...
 croak=>1 (lanza una excepci贸n en lugar de retornar un $self vacio)
...
sub new_old {
    ...
    if ($self) {
        ...
        if ($rc) {
            ...
            if ( $p{croak} ) {
                croak "Error de conexi贸n al agente en $p{host}:$p{port}";
            }
            else {
                return undef;
            }
...
sub getFile {
    ...
    if ( $RC ne 0 ) {
        chop $RET;
        croak
"Error de lectura del fichero '$rfile' en la m谩quina '$balix->{RM}': $RET";
    }
...
####IMPRESCINDIBLE QUE BALIX SE EJECUTE COMO ROOT.  SI NO, PIDE PASSWORD.
    ...
    if ( $user eq "" ) {
        return ( 99,
            "ERROR DE BALIX: usuario de ejecuci贸n remota est谩 en blanco!" );
    }
    my $socket = $balix->{socket};
...
sub ping {
    ...
    if ( $byte ne substr( $hostname, 0, 1 ) ) {
        $RET =
"El servidor no responde correctamente.  Verifique la versi贸n de balix instalada";
        $RC = 1;
    }
