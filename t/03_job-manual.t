...
our $value;
{
    ...
    register 'service.test.elements' => { 
        handler=>sub{ 
            my ($self,$c,$config)=@_;
            my $job = $c->stash->{job};
            my $stash = $job->job_stash;
            use BaselinerX::Job::Elements;
            use BaselinerX::Job::GenericElement;
            my $es = new BaselinerX::Job::Elements;
            $es->push_element( BaselinerX::Job::GenericElement->new(path=>'/Project1/FILES/f1', name=>'f1', mask=>'/app/nat/sub' ) );
            $stash->{elements} = $es; 
        }
    };
...
}
{
    ...
    $chain->bali_chained_services->create({ seq=>100, key=>'service.test.elements', step=>'PRE'  });
    $chain->bali_chained_services->create({ seq=>200, key=>'service.manual_deploy.request', step=>'PRE'  });
    #$chain->bali_chained_services->create({ seq=>200, key=>'service.manual_deploy', step=>'PRE'  });
    ...
    $mu->create({ username=>'rptuser', password=>'password' });
    Baseliner->model('Permissions')->create_role( 'RPT' ); 
    Baseliner->model('Permissions')->add_action( 'action.manual.approve.files', 'RPT' ); 
    Baseliner->model('Permissions')->grant_role( username=>'rptuser', role=>'RPT' ); 
...
if(0) {
    ...
    $job = $job->get_from_storage;  # reload row
    is( $main::value, 1001, "stash value ok" );
