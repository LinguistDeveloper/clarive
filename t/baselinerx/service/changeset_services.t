...
subtest 'sync_baselines: does nothing when job failed' => sub {
    _setup();

    my $bl1 = TestUtils->create_ci( 'bl', name => 'QA',   bl => 'QA',   moniker => 'QA' );
    my $bl2 = TestUtils->create_ci( 'bl', name => 'TEST', bl => 'TEST', moniker => 'TEST' );
    ...
    $service->sync_baselines( $c, { bls => [ $bl1->mid, $bl2->mid ] } );

    ok !$repo->mocked_called('update_baselines');
};

subtest 'sync_baselines: does nothing no bls matched' => sub {
    _setup();
...
subtest 'sync_baselines: syncs baseline' => sub {
    _setup();
    ...
    TestGit->tag( $repo, tag => 'TEST' );
    TestGit->commit($repo);
    my $qa_sha = TestGit->commit($repo);
    my $sha    = TestGit->commit($repo);
    TestGit->tag( $repo, tag => 'QA' );
    my $rev = TestUtils->create_ci( 'GitRevision', repo => $repo, sha => $sha );

    my $job = _mock_job( bl => sub { 'QA' } );
    my $c = _mock_c(
        stash => {
            job             => $job,
            project_changes => [
    ...
    is( TestGit->rev_parse( $repo, 'QA' ),   $sha );
    is( TestGit->rev_parse( $repo, 'TEST' ), $sha );
};

subtest 'sync_baselines: rollbacks synced baseline' => sub {
    _setup();
    ...
    my $job = _mock_job( bl => sub { 'QA' }, rollback => sub { 1 } );
    my $c = _mock_c(
        stash => {
            bl_sync => {
                $repo->mid => {
    ...
    is( TestGit->rev_parse( $repo, 'TEST' ), $prev_sha );
};
...
sub _mock_job {
    ...
    $job->mock( bl => $params{bl} || sub { 'TEST' } );
