#INFORMACI脫N DEL CONTROL DE VERSIONES
#
#	CAM .............................. SCM
#	Pase ............................. N.ANTE0000070494
#	Fecha de pase .................... 2012/09/12 19:26:32
#	Ubicaci贸n del elemento ........... /SCM/FICHEROS/UNIX/baseliner/features/sqa/lib/BaselinerX/Model/SQA.pm
#	Versi贸n del elemento ............. 9
#	Propietario de la versi贸n ........ q73898x (Q73898X - LUIS MIGUEL GARCIA MANCEBO)
...
qq{cd /D $config->{script_dir} & call ant -f $config->{script_name} Subaplicacion -Dsubapp="$subproject" -Dentorno=$bl -DCAM="$CAM" -Dproyecto="$project" -DinputDir="$dir_pase"};
	...
	unless ($rc) {
		...
		#Comentado lo anterior, se env韆 notificaci髇 del fallo a SCM y SQA
	...
	if ($data) {
		...
		if ( $nature =~ /NET|J2EE/ ) {
			...
			if ( $nature =~ /NET/ && $mstest ) {
				$mstest =~ s/\"//g; #en la nueva versi髇 del fichero .csv los caracteres vienen entre comillas, las eliminamos. REVISAR posteriores versiones si no se pintan bien los datos
				@fichero             = split "\n", $mstest;
				...
				$junit =~ s/\"//g; #en la nueva versi髇 del fichero .csv los caracteres vienen entre comillas
				@fichero             = split "\n", $junit;
...
sub request_analysis {
	...
	my $user       = $p{user};
	my $bl         = $p{bl};
	my $job_id     = $p{job_id};  # infroox: SQAScheduler no manda job_id nunca, sino id que 脗驴viene a ser lo mismo?
	my $return     = 1;
...
sub getProjectConfigAll {
	...
	if ($row_subnat) {    # Hay fila de proyecto busco su configuraci贸n
		$row_subnat_config =
		  Baseliner->model('Baseliner::BaliConfig')
		  ->search(
			{ bl => $bl, ns => 'project/' . $row_subnat->mid, key => $value } )
		  ->first;
		if ($row_subnat_config) {    # Si hay configuraci贸n, la uso
			$return = $row_subnat_config->value;
			_log "************ CONFIGURACION DE SUBAPLICACION/NATURALEZA";
		}
		else
		{   # No hay configuraci贸n, busco la configuraci贸n de CAM/naturaleza
			...
			{    # Hay configuraci贸n CAM/Naturaleza.  La uso
				$return = $row_camnat_config->value;
				_log "************ CONFIGURACION DE CAM/NATURALEZA";
			}
			else
			{ # No hay configuraci贸n de CAM/Naturaleza.  Uso la de subaplicaci贸n
				_log "************ NO HAY DE CAM/NATURALEZA";
				if ($row_subproject)
				{    # Hay fila de subproyecto. Busco su configuraci贸n
					...
					{    #Hay configuraci贸n de subproyecto.  La uso
						$return = $row_subproject_config->value;
						_log "************ CONFIGURACION DE SUBAPLICACION";
					}
					else
					{  #No hay configuraci贸n de subproyecto.  Busco la del CAM
						...
						{    # Hay configuraci贸n del CAM.  La uso
							$return = $row_project_config->value;
							_log "************ CONFIGURACION DE CAM";
						}
						else
						{ # No hay configuraci贸n del CAM.  Uso de de la naturaleza global.  Si no hay se usar谩 la global
...
"************ CONFIGURACION GLOBAL DE NATURALEZA";
							...
							{ # No hay configuraci贸n global de naturaleza.  Uso la global.
				...
				{   # No hay fila de subproyecto. Uso la configuraci贸n del CAM
					$row_project_config =
					  Baseliner->model('Baseliner::BaliConfig')->search(
						{
							bl  => $bl,
...
"************ CONFIGURACION GLOBAL DE NATURALEZA";
	...
	{ # No hay fila de subaplicaci贸n/naturaleza, busco la configuraci贸n de CAM/naturaleza
		...
		if ($row_camnat_config) {  # Hay configuraci贸n CAM/Naturaleza.  La uso
			$return = $row_camnat_config->value;
			_log "************ CONFIGURACION DE CAM/NATURALEZA";
		}
		else
		{ # No hay configuraci贸n de CAM/Naturaleza.  Uso la de subaplicaci贸n
			...
			{    # Hay fila de subproyecto. Busco su configuraci贸n
				...
				else { #No hay configuraci贸n de subproyecto.  Busco la del CAM
					$row_project_config =
					  Baseliner->model('Baseliner::BaliConfig')->search(
						{
							bl  => $bl,
...
"************ CONFIGURACION GLOBAL DE NATURALEZA";
						...
						{ # No hay configuraci贸n global de naturaleza.  Uso la global.
							$row_global =
							  ...
							  ->first;
							if ($row_global)
							{    # Hay configuraci贸n global.  La uso
								$return = $row_global->value;
								_log "************ CONFIGURACION GLOBAL";
							}
						}
			...
			else {  # No hay fila de subproyecto. Uso la configuraci贸n del CAM
				$row_project_config =
				  Baseliner->model('Baseliner::BaliConfig')->search(
					{
						bl  => $bl,
				...
				{ # No hay configuraci贸n del CAM.  Uso de de la naturaleza global.  Si no hay se usar谩 la global
					...
					{ # No hay configuraci贸n global de naturaleza.  Uso la global.
						$row_global =
						  ...
						  ->first;
						if ($row_global) { # Hay configuraci贸n global.  La uso
							$return = $row_global->value;
							_log "************ CONFIGURACION GLOBAL";
						}
					}
...
sub getProjectConfigAll_old {
	...
	if ($row) {
		...
		if ( $config->{$value} ) {
			$return = $config->{$value};
			_log "****** Nivel: subaplicaci贸n/naturaleza";
			_dump $config;
		}
