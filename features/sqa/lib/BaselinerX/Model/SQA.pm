...
#	Propietario de la versión ........ q73898x (Q73898X - LUIS MIGUEL GARCIA MANCEBO)
...
sub request_analysis {
	...
	my $user       = $p{user};
	my $bl         = $p{bl};
	my $job_id     = $p{job_id};  # infroox: SQAScheduler no manda job_id nunca, sino id que Â¿viene a ser lo mismo?
	my $return     = 1;
...
sub getProjectConfigAll {
	...
	if ($row_subnat) {    # Hay fila de proyecto busco su configuración
		$row_subnat_config =
		  Baseliner->model('Baseliner::BaliConfig')
		  ->search(
			{ bl => $bl, ns => 'project/' . $row_subnat->mid, key => $value } )
		  ->first;
		if ($row_subnat_config) {    # Si hay configuración, la uso
			$return = $row_subnat_config->value;
			_log "************ CONFIGURACION DE SUBAPLICACION/NATURALEZA";
		}
		else
		{   # No hay configuración, busco la configuración de CAM/naturaleza
			...
			{    # Hay configuración CAM/Naturaleza.  La uso
				$return = $row_camnat_config->value;
				_log "************ CONFIGURACION DE CAM/NATURALEZA";
			}
			else
			{ # No hay configuración de CAM/Naturaleza.  Uso la de subaplicación
				_log "************ NO HAY DE CAM/NATURALEZA";
				if ($row_subproject)
				{    # Hay fila de subproyecto. Busco su configuración
					...
					{    #Hay configuración de subproyecto.  La uso
						$return = $row_subproject_config->value;
						_log "************ CONFIGURACION DE SUBAPLICACION";
					}
					else
					{  #No hay configuración de subproyecto.  Busco la del CAM
						...
						{    # Hay configuración del CAM.  La uso
							$return = $row_project_config->value;
							_log "************ CONFIGURACION DE CAM";
						}
						else
						{ # No hay configuración del CAM.  Uso de de la naturaleza global.  Si no hay se usará la global
...
"************ CONFIGURACION GLOBAL DE NATURALEZA";
							...
							{ # No hay configuración global de naturaleza.  Uso la global.
				...
				{   # No hay fila de subproyecto. Uso la configuración del CAM
					$row_project_config =
					  Baseliner->model('Baseliner::BaliConfig')->search(
						{
							bl  => $bl,
...
"************ CONFIGURACION GLOBAL DE NATURALEZA";
	...
	{ # No hay fila de subaplicación/naturaleza, busco la configuración de CAM/naturaleza
		...
		if ($row_camnat_config) {  # Hay configuración CAM/Naturaleza.  La uso
			$return = $row_camnat_config->value;
			_log "************ CONFIGURACION DE CAM/NATURALEZA";
		}
		else
		{ # No hay configuración de CAM/Naturaleza.  Uso la de subaplicación
			...
			{    # Hay fila de subproyecto. Busco su configuración
				...
				else { #No hay configuración de subproyecto.  Busco la del CAM
					$row_project_config =
					  Baseliner->model('Baseliner::BaliConfig')->search(
						{
							bl  => $bl,
...
"************ CONFIGURACION GLOBAL DE NATURALEZA";
						...
						{ # No hay configuración global de naturaleza.  Uso la global.
							$row_global =
							  ...
							  ->first;
							if ($row_global)
							{    # Hay configuración global.  La uso
								$return = $row_global->value;
								_log "************ CONFIGURACION GLOBAL";
							}
						}
			...
			else {  # No hay fila de subproyecto. Uso la configuración del CAM
				$row_project_config =
				  Baseliner->model('Baseliner::BaliConfig')->search(
					{
						bl  => $bl,
				...
				{ # No hay configuración del CAM.  Uso de de la naturaleza global.  Si no hay se usará la global
					...
					{ # No hay configuración global de naturaleza.  Uso la global.
						$row_global =
						  ...
						  ->first;
						if ($row_global) { # Hay configuración global.  La uso
							$return = $row_global->value;
							_log "************ CONFIGURACION GLOBAL";
						}
					}
...
sub getProjectConfigAll_old {
	...
	if ($row) {
		...
		if ( $config->{$value} ) {
			$return = $config->{$value};
			_log "****** Nivel: subaplicación/naturaleza";
			_dump $config;
		}
