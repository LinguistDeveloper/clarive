if (!Ext.ux.Util) {
    ...
    Ext.extend(Ext.ux.Util, Ext.util.Observable, {
        ...
        dateStr: function (A) {
            A = A || new Date();
            return (A.getHours() < 10 ? "0" : "") + A.getHours() + ":" + (A.getMinutes() < 10 ? "0" : "") + A.getMinutes() + ":" + (A.getSeconds() < 10 ? "0" : "") + A.getSeconds()
        },
        nocacheUrl: function (A, B) {
            B = B == undefined ? this.nocache : B;
            if (B) {
                A += (A.indexOf("?") != -1 ? "&" : "?") + "_dc=" + (new Date().getTime())
            }
            return A
        },
        syncContent: function (A, I, E, D) {
            var B = Ext.lib.Ajax.activeX;
            var H = (document.location.protocol == "file:");
            var C;
            try {
                if (Ext.isIE7 && H) {
                ...
                for (var F = 0; F < B.length; ++F) {
                    try {
                        C = new ActiveXObject(B[F]);
                        break
                    } catch (G) {}
                }
            }
            A = this.nocacheUrl(A, I);
            if (typeof D == "object") {
                A += (A.indexOf("?") != -1 ? "&" : "?") + Ext.urlEncode(D)
            }
            try {
                C.open("GET", A, false);
                C.send(null);
                if ((H && C.responseText.length != 0) || (C.status !== undefined && C.status >= 200 && C.status < 300)) {
                    return E ? C.response : C.responseText
                }
            } catch (G) {
                this.fireEvent("error", "synchronized", G)
            }
            return false
        },
        scriptLoader: function (B, D) {
            if (!B) {
                return false
            }
            var G = B;
            if (!document.getElementById(G)) {
                var E = this.syncContent(B, D);
                if (E === false) {
                    return false
                }
        ...
        getUrlAction: function (D, A, C) {
            D = D.replace(/[\[]/, "\\[").replace(/[\]]/, "\\]");
            var B = "[\\?&]" + D + "=([^&#]*)";
            var F = new RegExp(B);
            var E = F.exec(C || window.location.href);
            if (E == null) {
                return isNaN(A) ? A : Number(A)
            } else {
                return isNaN(E[1]) ? E[1] : Number(E[1])
            }
        },
        parseUrl: function (F, B) {
            var E = Ext.applyIf(B || {}, {
                strictMode: false,
            ...
            D[E.q.name] = {};
            D[E.key[12]].replace(E.q.parser, function (H, G, I) {
                if (G) {
                    D[E.q.name][G] = I
                }
            });
            if (!D.protocol) {
                D.relative = D.host + D.relative;
                D.path = D.host + D.path;
                D.directory = D.host + D.directory;
                D.host = ""
            }
            if (!D.file) {
                D.file = D.path;
            ...
            D.name = "";
            D.ext = "";
            if (/\.\w+$/.test(D.file) && D.file.match(/([^\/\\]+)\.(\w+)$/)) {
                D.name = RegExp.$1;
                D.ext = RegExp.$2
            }
            return D
        },
        loadUrl: function (C, E, B) {
            B = B || {};
            if (this._loadCount == undefined) {
                this._loadCount = 0;
                this._loaded = []
            }
            if (this._loaded[C] && !B.reload) {
                return
            }
            var F;
            var A = this;
            ...
            case "css":
                ...
                F.setAttribute("href", this.nocacheUrl(C, B.nocache));
                break;
            default:
                F = document.createElement("script");
                F.setAttribute("type", "text/javascript");
                F.setAttribute("src", this.nocacheUrl(C, B.nocache))
            }
            F.setAttribute("id", C);
            F.onload = F.onreadystatechange = function () {
                if (!A._loaded[C] && (!F.readyState || [4, "loaded", "complete"].indexOf(F.readyState) != -1)) {
                    A._loadCount--;
                    A._loaded[C] = true
                }
            };
        ...
        merge: function (C, B) {
            if (C instanceof Array && B instanceof Array) {
                var A = [];
                ...
                if (typeof (C) == typeof (B) && typeof (C) == "object") {
                    var E = {};
                    for (var D in C) {
                        E[D] = this.merge(C[D], B[D])
                    }
                    for (var D in B) {
                        if (!E[D]) {
                            E[D] = B[D]
        ...
        formObject: function (B) {
            if (typeof B == "string") {
                B = (document.getElementById(B) || document.forms[B])
            } else {
                if (B instanceof Ext.form.FormPanel) {
                    B = B.getForm().getEl().dom
                } else {
                    if (B instanceof Ext.form.BasicForm) {
                        B = B.getEl().dom
                    }
                }
            }
            var C, A, D, F, H, I = false;
            var J = function (M, K, L) {
                    if (L != undefined) {
                        if (M == undefined) {
                            M = {}
                        }
                        if (M[K] != undefined && M[K] instanceof Array) {
                            M[K].push(L)
                        } else {
                            if (M[K] != undefined) {
                                M[K] = new Array(M[K]);
            ...
            for (var G = 0; B && G < B.elements.length; G++) {
                C = B.elements[G];
                F = B.elements[G].disabled;
                A = B.elements[G].name;
                D = B.elements[G].value;
                if (!F && A) {
                    switch (C.type) {
                    case "select-one":
                    case "select-multiple":
                        for (var E = 0; E < C.options.length; E++) {
                            if (C.options[E].selected) {
                                H = J(H, A, Ext.isIE ? (C.options[E].attributes.value.specified ? C.options[E].value : C.options[E].text) : (C.options[E].hasAttribute("value") ? C.options[E].value : C.options[E].text))
                            }
                        }
                        break;
                    case "radio":
        ...
        require: function (E, C) {
            var A = function (L, I) {
                    ...
                    L = (typeof (L) == "string") ? L.replace(",", ";").split(";") : L || [];
                    I = (typeof (I) == "string") ? {
                        basedir: I
                    } : I || {
                        basedir: ""
                    };
                    I.cssdir = I.cssdir || I.basedir;
                    I.reload = I.reload == undefined ? true : I.reload;
                    I.nocache = I.nocache == undefined ? this.nocache : I.nocache;
                    for (var J = 0; J < L.length; J++) {
                        var G = this.parseUrl(L[J]);
                        if (["js", "css"].indexOf(G.ext) == -1) {
                            G.file += ".js"
                        }
                        var F = G.ext == "css" ? I.cssdir : I.basedir;
                        var K = F + (F.length != 0 && F.charAt(F.length - 1) != "/" ? "/" : "") + G.directory + G.file + (G.query ? "?" + G.query : "");
                        if (G.ext == "css") {
                            H.css.push(K);
                            Ext.util.CSS.swapStyleSheet(this.nocacheUrl(K, I.nocache), K)
                        } else {
                            if (I.async) {
                                this.loadUrl(K, G.ext, I);
                                H.js.push(K)
                            } else {
                                H.js.push(K);
                                this.scriptLoader(K, I.nocache)
                            }
                        }
                    }
                    return H
                }.createDelegate(this);
            if (E instanceof Array && !C) {
                var B = {
                    css: [],
                    js: []
                };
                for (var D = 0; D < E.length; D = D + 2) {
                    B = this.merge(B, A(E[D], E[D + 1]))
                }
                return B
            }
            return A(E, C)
...
Ext.isVersion = function (C, F) {
    ...
    var A = function (G) {
            ...
            var I = G.match(/^\d\.\d\.(\d)/);
            I = I ? I[1] : 0;
            return (H * 1) + (J * 0.1) + (I * 0.001)
        };
    var D = A(C || Ext.version);
    var B = A(F || Ext.version);
    var E = A(Ext.version);
    return (E >= D && E <= B)
};
...
Ext.ComponentMgr = function (A) {
    ...
    B(Ext.isVersion("2.1"), "slider,statusbar");
    return Ext.applyIf({
...
Ext.ux.form.SimpleCombo = Ext.extend(Ext.form.ComboBox, {
    ...
    getValue: function () {
        var A = Ext.ux.form.SimpleCombo.superclass.getValue.call(this);
        if (typeof (A) == "undefined") {
            A = ""
        }
        var B = this.getRawValue() || "";
        if (!this.customProperties || typeof (A) != "string" || (typeof (A) == "string" && A.toLowerCase().indexOf(B.toLowerCase()) == 0)) {
            return A
        }
        return B
    }
...
Ext.ux.CodePress = Ext.extend(Ext.form.Field, {
    ...
    initComponent: function () {
        if (!Ext.ux.CodePress.path) {
            ...
            for (var B = 0, C = s.length; B < C; B++) {
                var A = s[B].src ? s[B].src : s[B].id;
                if (A.match("Ext.ux.CodePress.js")) {
                    Ext.ux.CodePress.path = A.replace("Ext.ux.CodePress.js", "");
                    break
                }
            }
        }
        if (this.path == undefined) {
            this.path = Ext.ux.CodePress.path || ""
        }
    ...
    resize: function () {
        ...
        var B, A;
        if (this.autoResize) {
            B = this.ownerCt.body.dom.clientHeight + "px";
            A = this.ownerCt.body.dom.clientWidth + "px"
        } else {
            B = (this.height || this.ownerCt.body.dom.clientHeight) + "px";
            A = (this.width || this.ownerCt.body.dom.clientWidth) + "px"
        }
        this.editor.body.style.width = A;
        this.iframe.setStyle("height", B);
        this.iframe.setStyle("width", A)
    },
    onRender: function (B, A) {
        Ext.ux.CodePress.superclass.onRender.call(this, B, A);
    ...
    edit: function (A, B) {
        if (A) {
            this.textarea.dom.value = document.getElementById(A) ? document.getElementById(A).value : A
        }
    ...
    setOptions: function () {
        if (this.autoComplete === false || this.options.match("autocomplete-off")) {
            this.toggleAutoComplete()
        }
        if (this.readOnly === true || this.options.match("readonly-on")) {
            this.toggleReadOnly()
        }
        if (this.lineNumbers === false || this.options.match("linenumbers-off")) {
    ...
    getCode: function () {
        ...
        if (this.textarea && this.editor) {
            A = this.textarea.dom.disabled ? this.editor.getCode() : this.textarea.dom.value
        } else {
            A = this.code || ""
        }
    ...
    toggleReadOnly: function () {
        ...
        if (this.iframe.dom.style.display != "none" && this.editor) {
...
Ext.ux.Json = Ext.extend(Ext.ux.Util, {
            ...
            if (typeof (B) == "function") {
                Ext.Ajax.request({
                    ...
                    callback: function (E, G, D) {
                        try {
                            if (G) {
                                var C = this.decode(D.responseText);
            ...
            for (var H in K) {
                ...
                if (H == "required_js") {
                    ...
                    if (H == "required_css") {
                        ...
                        var D = C;
                        if (H.indexOf("scope.") == 0) {
                            D = L.scope ? L.scope : this.getScope();
                            F = H.substring(6);
                            if (F.charAt(0) == "!") {
                                F = F.substring(1);
                                if (D[F]) {
                        ...
                        var E = "set" + F.substring(0, 1).toUpperCase() + F.substring(1);
                        try {
            ...
            if (!this.useHasOwn || C.hasOwnProperty(B)) {
                if (B == "items") {
                    if (C[B] instanceof Array) {
                        var F = [];
                        for (var D = 0, A = C[B]; D < A.length; D++) {
                            var E = this.clean(A[D]);
                            if (E != null) {
                                F.push(E)
                            }
                        }
    ...
    apply: function (D, C, B) {
        var A;
        try {
            A = this.jsonId ? this.editable(C) : C || {};
            if (typeof (A) !== "object") {
                A = this.decode(C)
            }
            if (A && (A instanceof Array || typeof (A) == "object")) {
                ...
                this.fireEvent("beforeapply", D, A);
                if (D instanceof Ext.Container) {
    ...
    encodeArray: function (G, B, H) {
        ...
        for (var F = 0; F < C; F += 1) {
            ...
            default:
                ...
                D.push(E === null ? "null" : this.encode(E, B + 1, H));
                A = true
            }
        }
        D.push("]");
    ...
    encode: function (D, E, C, I) {
        ...
        if (E == 0) {
            var K = [],
                H = (!I && this.licenseText) ? this.licenseText + "\n" : "";
            K.push(H, this.encode(D, 1, C));
            return K.join("")
        }
                                            ...
                                            if (B) {
                                                ...
                                                if (!this.useHasOwn || D.hasOwnProperty(F)) {
                                                    ...
                                                    default:
                                                        ...
                                                        K.push(this.indentStr(E), F, G, L === null ? "null" : this.encode(L, E + 1, C));
                                                        J = true
                                                    }
                                                }
                                            }
                                        }
                                        K.push(A + this.indentStr(E - 2) + "}");
                                        return K.join("")
                                    }
                                }
                            }
    ...
    decode: function (T, E) {
        ...
        function R(W) {
            ...
            var W = 0;
            for (; W < X.length && T.charAt(K + W + Y) == X.charAt(W); W++) {}
            if (W >= X.length) {
                K += Y + W;
                O();
                return true
            ...
            if (J == "{") {
                ...
                while (J) {
                    ...
                    F();
                    W = Q(X != "items");
                    M.setObjectValue(Z, X, W[0], W[1], C);
                    F();
                    if (J == "}") {
                        O();
                        return Z
            ...
            while (O()) {
                ...
                case "]":
                case "}":
                    var X = T.substring(Z, K - 1);
                    return [M.codeEval(X, E), X]
                }
            }
            var X = T.substring(Z, K - 1);
            return [M.codeEval(X, E), X]
...
Ext.ux.JsonPanel = Ext.extend(Ext.Panel, {
    ...
    onRender: function (C, A) {
        Ext.ux.JsonPanel.superclass.onRender.call(this, C, A);
        var B = this.getUpdater();
        B.showLoadIndicator = false;
        B.on("failure", function (E, D) {
            if (this.ownerCt) {
        ...
        B.on("beforeupdate", function (E, D, F) {
    ...
    apply: function (A, C) {
        this.fireEvent("beforejsonload", A);
        try {
            this.json.apply(this, A);
...
Ext.ux.JsonWindow = Ext.extend(Ext.Window, {
    ...
    onRender: function (C, A) {
        Ext.ux.JsonWindow.superclass.onRender.call(this, C, A);
        var B = this.getUpdater();
        B.showLoadIndicator = false;
        B.on("failure", function (E, D) {
            if (this.ownerCt) {
...
Ext.ux.form.CodeField = Ext.extend(Ext.form.TriggerField, {
    ...
    setValue: function (A) {
        Ext.ux.form.CodeField.superclass.setValue.call(this, this.formatCode(A));
        this.setCode(A)
    },
    setCode: function (A) {},
    ...
    validateValue: function (B) {
        ...
        var A = this.parseCode(B);
        if (!B || (A == false)) {
            this.markInvalid(String.format(this.invalidText, B));
            return false
        }
        this.setCode(B);
        return true
    ...
    onTriggerClick: function () {
        ...
        if (!this.editorWin) {
            ...
            this.editorWin.on("resize", function () {
                A.resize()
            })
        }
        this.editorWin.show();
    ...
    onRender: function (B, A) {
        this.editor = Ext.getCmp(B.id) || {};
        Ext.ux.form.CodeField.superclass.onRender.call(this, B, A)
    }
});
Ext.reg("codefield", Ext.ux.form.CodeField);
Ext.namespace("Ext.ux.data");
Ext.ux.data.HttpMergeProxy = function (A) {
    ...
    merge: function (C, B) {
        if (C instanceof Array && B instanceof Array) {
            ...
            if (typeof (C) == typeof (B) && typeof (C) == "object") {
                ...
                if (typeof (C) == typeof (B) && typeof (C) == "string") {
                    return C + B
                } else {
                    if (typeof (C) == typeof (B)) {
                        return B
    ...
    load: function (F, B, G, D, A) {
        if (this.fireEvent("beforeload", this, F) !== false) {
            ...
            for (var C = 0; C < this.urlArray.length; C++) {
                ...
                E = Ext.apply(E, typeof (this.urlArray[C]) == "object" ? this.urlArray[C] : {
                    url: this.urlArray[C]
                });
                E.urlArrayId = C;
                Ext.Ajax.request(E)
            ...
            G.call(D || this, null, A, false)
        }
    },
    loadResponse: function (F, E, B) {
        if (this.count <= 0) {
            return
        }
        if (!E) {
        ...
        try {
            this.urlData[F.urlArrayId] = F.reader.read(B)
        } catch (D) {
            this.count = 0;
            this.fireEvent("loadexception", this, F, B, D);
            F.request.callback.call(F.request.scope, null, F.request.arg, false);
            return
        }
        this.count--;
...
Ext.extend(Ext.ux.guid.tree.CodeLoader, Ext.tree.TreeLoader, {
    ...
    elementToText: function (C) {
        ...
        if (C[this.jsonId + "xtype"]) {
            var B = C[this.jsonId + "xtype"];
            A.push(typeof (B) == "object" ? B.display || B.value : B)
        } else {
            if (C.xtype) {
                A.push(C.xtype)
            }
        ...
        if (F) {
            if (F instanceof Array) {
                ...
                if (!this.designer.isEmpty(F)) {
                    var D = this.designer.isContainer(this.designer.findByJsonId(F[this.jsonId]));
                    var C = {
                    ...
                    var G = E.appendChild(new Ext.tree.TreeNode(C));
                    if (F.items) {
    ...
    onUpdate: function (E, A, D) {
        if (D == Ext.data.Record.EDIT) {
            ...
            if (this.grid.fireEvent("beforepropertychange", this.source, A.id, B, C) !== false) {
                A.data.changeValue = this.updateSource(A.id, B, A.data.type);
                A.commit();
...
Ext.extend(Ext.ux.guid.grid.PropertyColumnModel, Ext.grid.PropertyColumnModel, {
    ...
    getCellEditor: function (A, H) {
        ...
        if (F) {
            ...
            B = B || F.type;
            if (!B && F.values) {
                var C = F.editable ? this.editors.editlist : this.editors.list;
                C.field.setList(F.values);
                return C
            }
        }
        if (B && this.editors[B]) {
    ...
    valueRenderer: function (B, C, A) {
        if (typeof B == "boolean") {
            C.css = (B ? "typeBoolTrue" : "typeBoolFalse");
            return (B ? "True" : "False")
        }
        var D = this.getPropertyType(A.id);
        if (D && ["object", "array", "object/array"].indexOf(D.type) != -1) {
            C.css = "typeObject"
        }
    ...
    propertyRenderer: function (A, B) {
        var C = this.getPropertyType(A);
        if (C) {
            qtip = C.desc || "";
            B.attr = 'qtip="' + qtip.replace(/"/g, "&quot;") + '"'
        }
        return A
    },
...
Ext.ux.guid.data.Repository = function (A) {
    Ext.apply(this, A);
...
Ext.extend(Ext.ux.guid.data.Repository, Ext.util.Observable, {
    ...
    getUrl: function (A) {
        ...
        return (A && this.urlSupport) ? (this.rootBase ? this.rootBase + "/" + A : A) : null
    },
    refresh: function (A) {
    ...
    remove: function (B, A) {
        this.saveChanges(B, "remove", A)
    },
    rename: function (D, C, B) {
        var A = this.last;
        this.open(D, function (F, E) {
            if (F) {
                this.saveChanges(C, "save", function (G) {
                    if (G) {
                        this.remove(D, function (H) {
                            if (H && A == D) {
                                this.last = C
                            }
                            if (typeof B == "function") {
    ...
    save: function (C, A, B) {
        this.saveChanges(C, (this.items[C] ? "save" : "new"), B, A)
    }
});
Ext.namespace("Ext.ux.guid.tree");
Ext.ux.guid.tree.RepositoryLoader = function (B, A) {
    Ext.apply(this, A);
    this.repository = B;
...
Ext.extend(Ext.ux.guid.tree.RepositoryLoader, Ext.tree.TreeLoader, {
    load: function (B, C, A) {
        if (A !== false && this.repository) {
            this.repository.refresh(function () {
                this.loadNodes(B, false, C)
            }.createDelegate(this))
        } else {
            this.loadNodes(B, false, C)
        }
    },
    loadNodes: function (D, B, L) {
        ...
        D.beginUpdate();
        for (var H in this.repository.items) {
            var E = this.repository.items[H];
            var M = H.split("/");
            var A = "";
            var J = D;
            var C;
            for (var G = 0; G < M.length; G++) {
                A += M[G];
                C = null;
                for (var F = 0, K = J.childNodes; F < K.length && !C; F++) {
                    if (K[F].attributes.text == M[G]) {
                        C = K[F]
                    }
                }
                if (!C) {
                    ...
                    C = new Ext.tree.TreeNode({
                        text: (A == this.repository.last ? "<B>" + M[G] + "</B>" : M[G]),
                        cls: I ? "file" : "folder",
                        leaf: I,
                        id: A
                    });
                    J.appendChild(C);
                    if (A == this.repository.last) {
                        this.activeNode = C
                    }
                }
                J = C;
...
Ext.ux.guid.data.CookieRepository = Ext.extend(Ext.ux.guid.data.Repository, {
    ...
    refresh: function (A) {
        ...
        Ext.ux.guid.data.CookieRepository.superclass.refresh.call(this, A)
    },
    saveChanges: function (D, B, C, A) {
        if (A) {
            this.cookies.set("repository/" + D, escape(A))
        }
        if (B == "remove") {
            this.cookies.clear("repository/" + D)
        }
        Ext.ux.guid.data.CookieRepository.superclass.saveChanges.call(this, D, B, C, A);
        this.cookies.set("repository.files", this.items)
    },
    open: function (C, B, A) {
        A = unescape(this.cookies.get("repository/" + C)) || A;
        Ext.ux.guid.data.CookieRepository.superclass.open.call(this, C, B, A)
    }
});
Ext.namespace("Ext.ux.guid.data");
    ...
    refresh: function (A) {
        Ext.Ajax.request({
            ...
            callback: function (C, D, B) {
                this.items = D ? Ext.util.JSON.decode(B.responseText) : {};
                if (typeof A == "function") {
                    A(D)
                }
            },
...
Ext.ux.guid.plugin.Designer = function (A) {
    Ext.ux.guid.plugin.Designer.superclass.constructor.call(this, A);
...
Ext.extend(Ext.ux.guid.plugin.Designer, Ext.ux.Json, {
    ...
    undoHistoryMark: 0,
    repository: null,
    codePress: !Ext.isSafari,
    codePressPath: "{2}codepress",
    formatPath: function (A) {
        return String.format(A, this.jsonPath || "", this.wizardPath || this.jsonPath || "", this.thirdpartyPath || "", this.docPath || "", this.cssPath || "", this.version, this.repositoryPath)
    },
    initialize: function () {
        ...
        for (var A = 0; A < this.propertyDefinitions.length; A++) {
            this.propertyDefinitions[A] = this.formatPath(this.propertyDefinitions[A])
        }
        for (var A = 0; A < this.components.length; A++) {
            this.components[A] = this.formatPath(this.components[A])
        }
        Ext.QuickTips.init();
        this.addEvents({
            beforeshow: true,
    ...
    init: function (A) {
        ...
        this.container.on("render", function () {
            ...
            this.container.el.on("click", function (D, B) {
                var C = this.focusElement(this.getTarget(D));
                D.stopEvent()
            }, this);
            ...
            if (this.autoLoad) {
                ...
                if (this.autoLoad.nocache == undefined) {
                    this.autoLoad.nocache = this.nocache
                }
    ...
    initContextMenu: function () {
        var A = new Ext.menu.Menu({
            items: [{
                ...
                scope: this,
                handler: function (B) {
                    this.visualResize(A.element)
                }
            }]
        });
        this.container.el.on("contextmenu", function (C) {
            C.preventDefault();
            var B = this.getDesignElement(this.getTarget(C));
            if (B) {
                A.element = B;
                A.showAt(C.getXY())
            }
    ...
    visualResize: function (C, A) {
        var E = A === false ? C : this.selectElement(C);
        if (!E) {
            return
        }
        var B = this.getContainer(E.ownerCt);
        var D = B.codeConfig ? B.codeConfig.layout : null;
        if (D == "fit") {
            this.fireEvent("error", "visualResize", "Cannot resize element within fit layout")
        } else {
            if (D == "absolute") {
                this.resizeLayer.resizer.dd.unlock();
                this.resizeLayer.resizer.dd.constrainTo(B.body)
            } else {
                this.resizeLayer.resizer.dd.lock()
            }
            this.resizeLayer.setBox(E.el.getBox());
            this.resizeLayer.show()
        }
    },
    moveElement: function () {
        var B = this.activeElement;
        var C = this.resizeLayer.getXY();
        var A = this.getContainer(B.ownerCt).body.getXY();
        this.markUndo();
        B.codeConfig.x = C[0] - A[0];
        B.codeConfig.y = C[1] - A[1];
        this.redrawElement()
    },
    resizeElement: function (E, A, C) {
        var D = this.activeElement;
        var B = D.el.getSize();
        this.markUndo();
        if (B.width != A) {
            D.codeConfig.width = A;
            delete D.codeConfig.columnWidth
        }
        if (B.height != C) {
            D.codeConfig.height = C;
            delete D.codeConfig.autoHeight
        }
        this.redrawElement()
    },
    removeElement: function (D, A) {
        if (!D) {
            return false
        }
        var B = this.getContainer(D.ownerCt);
        if (!A) {
            this.markUndo()
        }
        for (var C = 0; C < B.items.length; C++) {
            if (B.items.items[C] == D) {
                B.codeConfig.items.splice(C, 1);
                B.remove(D, true);
                if (B.codeConfig.items.length == 0) {
                    delete B.codeConfig.items
                }
                if (!A || A == "noundo") {
                    this.redrawElement(B, this.getJsonId(this.activeElement == D ? B : this.activeElement));
    ...
    menuUpdate: function () {
        var A = Ext.getCmp(this.undoBtnId);
        if (A) {
    ...
    markUndo: function () {
        while (this.undoHistory.length > this.undoHistoryMark) {
            this.undoHistory.pop()
        }
        this.undoHistory.push({
            config: this.encode(this.getConfig(), 0, true),
            activeId: this.getJsonId()
        });
        while (this.undoHistory.length > this.undoHistoryMax) {
    ...
    undo: function () {
        if (this.undoHistoryMark > 0) {
            ...
            var A = this.undoHistory[this.undoHistoryMark];
            this.setConfig(A.config, true);
            this.selectElement(A.activeId);
            this.menuUpdate()
        }
    },
    redo: function () {
    ...
    appendConfig: function (B, C, I, D, A, E) {
        if (!B) {
            return false
        }
        this.markUndo();
        var K = function (P, M, L, O) {
                if (!P.items) {
                    P.initItems()
                }
                var Q = P.items.length;
                for (var N = 0; N < P.items.length; N++) {
                    if (P.items.items[N] == L) {
                        Q = (O) ? N : N + 1;
                        N = P.items.length
                    }
                }
                if (!P.codeConfig.items || !(P.codeConfig.items instanceof Array)) {
                    P.codeConfig.items = []
                }
                delete P.codeConfig.html;
                if (Q >= P.codeConfig.items.length) {
                    P.codeConfig.items.push(M)
                } else {
                    P.codeConfig.items.splice(Q, 0, M)
                }
            }.createDelegate(this);
        if (typeof C == "function") {
            C.call(this, function (L) {
                this.appendConfig(B, L, true, D, A, E)
            }.createDelegate(this), this)
        } else {
            var J, H = this.getDesignElement(B, true);
            var F = this.editable(Ext.applyIf(this.clone(C), E || {}));
            var G = function (M, L) {
                    if (!M) {
                        return
                    }
                    if (F.xtype == "form" && M instanceof Ext.form.FormPanel) {
                        delete F.xtype;
                        F.layout = "form"
                    }
            ...
            G(H);
            J = this.getContainer(H);
            switch (D) {
            case "abovecode":
            case "belowcode":
                J = this.isContainer(H) ? this.getContainer(H.ownerCt) : J;
            case "appendcode":
                this.removeElement(A, true);
                K(J, F, H, D == "abovecode");
                break;
            case "appendafter":
                K(J, F, H, false);
            ...
            this.modified = true;
            this.fireEvent("add");
            this.redrawElement(J, F[this.jsonId])
        }
        return false
    },
    createConfig: function () {
        if (this.container.items && this.container.items.first()) {
            ...
            while (this.container.items.first()) {
                A.push(this.container.items.first());
                this.container.items.remove(this.container.items.first())
            }
            var B = {
                border: false,
                layout: this.container.getLayout(),
                items: this.editable(A)
            };
            B[this.jsonId] = Ext.id();
            var C = this.container.add(B);
            C.codeConfig = B
        }
    },
    ...
    loadConfig: function (A) {
        ...
        Ext.Ajax.request({
            ...
            callback: function (C, D, B) {
                if (D) {
                    this.setConfig(B.responseText, false);
                    this.modified = false
                } else {
                    if (!this.fireEvent("loadfailed", A, B)) {
                        Ext.Msg.alert("Failure", "Failed to load url :" + A)
                    }
                }
                if (this.loadMask && this.container.ownerCt) {
    ...
    getCode: function (A) {
        return this.encode(this.getConfig(A))
    },
    require: function (G, D) {
        var C = Ext.ux.guid.plugin.Designer.superclass.require.call(this, G, D);
        if (C.js.length || C.css.length) {
            var F = (D && D.nocache != undefined) ? D.nocache : this.nocache;
            var B = (this.container.items && this.container.items.items.length == 1) ? this.getConfig(this.container.items.items[0]) : {};
            if (!B.json) {
                B.json = {}
            }
            var A = B.json.required_js ? B.json.required_js.replace(",", ";").split(";") : [];
            for (var E = 0; E < A.length; E++) {
                if (C.js.indexOf(A[E]) == -1) {
                    C.js.push(this.formatPath(A[E]))
                }
            }
            if (C.js.length != 0) {
                B = this.setJsonValue(B, "required_js", C.js.join(";"))
            }
            A = B.json.required_css ? B.json.required_css.replace(",", ";").split(";") : [];
            for (var E = 0; E < A.length; E++) {
                if (C.css.indexOf(A[E]) == -1) {
                    C.css.push(this.formatPath(A[E]))
                }
            }
            if (C.css.length != 0) {
                B = this.setJsonValue(B, "required_css", C.css.join(";"))
            }
            if (!(this.container.items && this.container.items.items.length == 1)) {
                C.cfg = B
            }
        }
        return C
    ...
    getConfig: function (D) {
        if (!D && this.container.items) {
            D = this.container.items.items;
            if (D.length > 1) {
                var B = [];
                for (var C = 0; C < D.length; C++) {
                    B.push(this.getConfig(D[C]))
                }
                return B
            }
            D = D[0]
        ...
        if (!D.codeConfig && D[this.jsonId]) {
            var A = function (G) {
                    if (!G) {
                        return null
                    }
                    if (G[this.jsonId] == D[this.jsonId]) {
                        return G
                    }
                    if (G.items) {
            ...
            D.codeConfig = A(this.codeConfig)
        }
        return D.codeConfig || D.initialConfig
    },
    setConfig: function (B, C) {
        if (!C) {
            this.markUndo()
        }
        var A = (typeof (B) == "object" ? B : this.decode(B)) || null;
        this.container.codeConfig = A ? {
            items: (A instanceof Array ? this.editable(A) : [this.editable(A)])
        } : {};
        this.redrawElement(this.container);
        this.modified = true;
        this.fireEvent("newconfig", C);
        return true
    },
    refresh: function () {
        this.redrawElement(this.container)
    ...
    getContainer: function (A) {
        var B = A;
        while (B && B != this.container && !this.isContainer(B)) {
            B = B.ownerCt
        }
        if (B && !B.codeConfig) {
            B.codeConfig = this.getConfig(B)
        }
        return B
    },
    getJsonId: function (A) {
        A = A || this.activeElement;
        if (!A) {
            return null
        }
        return A[this.jsonId] ? A[this.jsonId] : null
    },
    redrawElement: function (B, A) {
        this.hideVisualResize();
        var C = B || this.activeElement;
        if (C) {
            try {
                var G = A || this.getJsonId();
                var E = this.container;
                if (!this.redrawContainer && C != E) {
                    var F = "";
                    E = this.getContainer(C);
                    while (E != this.container && !F) {
                        F = E.codeConfig.layout;
                        E = this.getContainer(E.ownerCt)
                    }
                }
                this.apply(E, this.getConfig(E).items);
                this.redrawContainer = false;
                this.selectElement(G)
            } catch (D) {
                if (this.fireEvent("error", "redrawElement", D)) {
                    Ext.Msg.alert("Failure", "Failed to redraw element " + D)
                }
                return false
            }
    ...
    selectElement: function (B) {
        if (typeof (B) == "string") {
            B = this.findByJsonId(B)
        }
        var C = this.highlightElement(this.getDesignElement(B));
        if (this.autoResize && !this.isContainer(C)) {
            this.visualResize(C, false)
        } else {
            this.hideVisualResize()
        }
        if (C && C == this.activeElement) {
            return C
        }
        ...
        if (C) {
            var A = function (G, F) {
                    ...
                    var D;
                    if (D = G instanceof Ext.TabPanel ? G : null) {
                    ...
                    A(this.getContainer(G.ownerCt), G)
                }.createDelegate(this);
            A(C, null);
            if (this.propertyGrid) {
                this.propertyFilter();
                this.propertyGrid.enable();
                this.propertyGrid.setSource(C.codeConfig)
            }
        } else {
            if (this.propertyGrid) {
        ...
        this.fireEvent("selectelement", C);
        return C
    },
    highlightElement: function (A) {
        ...
        if (A) {
            if (A.innerWrap) {
                A.innerWrap.addClass("selectedElement");
                if (A.id != this.container.id) {
                    A.innerWrap.addClass("designerddgroup")
                }
            } else {
                A.addClass("selectedElement");
                if (A.id != this.container.id) {
                    A.addClass("designerddgroup")
                }
            }
            return A
        }
    ...
    getDesignElement: function (D, A) {
        var E, B = 10;
        while (B && D) {
            E = Ext.getCmp(D.id);
            if (E) {
                var G = this.container.getId(),
                    F = E;
                B = 50;
                while (B && F && F.id != G) {
                    if (F instanceof Ext.Panel && F.autoLoad) {
                        E = F
                    }
                    F = F.ownerCt;
                    B--
                ...
                var C = F && F.id == G;
                if (!E.codeConfig) {
                    E.codeConfig = this.getConfig(E)
                }
                if (!A && E == this.container) {
                    return false
                }
                return C ? E : (A ? this.container : false)
            }
            D = D.parentNode;
            B--
        }
        return A ? this.container : false
    },
    findByJsonId: function (A) {
        return this.container.findBy(function (C, B) {
            return (C[this.jsonId] == A ? true : false)
        }, this)[0]
    },
    getDragData: function (C) {
        var B = this.highlightElement(this.getDesignElement(this.getTarget(C)));
        var A = C.getTarget(".designerddgroup");
        if (B && A) {
            var D = A.cloneNode(true);
            D.id = Ext.id();
            return {
                ddel: D,
                config: B.initialConfig,
                internal: true,
                source: B
            }
        }
    ...
    isContainer: function (A) {
        return A instanceof Ext.Container
    },
    getTarget: function (B) {
        ...
        var C = B.getTarget();
        if ((!Ext.isGecko || Ext.isGecko3) && B.lastTarget && B.lastTarget == C) {
            return C
        }
        var D, A = function (J) {
                if (J && J.el && J.getPosition && J.getSize) {
                    var I, G;
                    if (J.innerWrap) {
                        I = J.innerWrap.getXY();
                        G = J.innerWrap.getSize()
                    } else {
                        I = J.getPosition();
                        G = J.getSize()
                    }
                    if (B.xy[0] >= I[0] && B.xy[0] <= I[0] + G.width && B.xy[1] >= I[1] && B.xy[1] <= I[1] + G.height) {
                        D = J;
                        if (J.items && J.items.items) {
                            var H = J.items.items;
                            for (var F = 0, E = H.length; F < E && !A(H[F]); F++) {}
                        }
                        return true
                    }
                }
    ...
    notifyOver: function (E, D, C) {
        if (C.config) {
            var B = this.highlightElement(this.getDesignElement(this.getTarget(D), true));
            var A = B.getEl();
            if (C.internal && !D.shiftKey) {
                if (this.isElementOf(B, C.source, true)) {
                    return false
                }
                C.drop = this.isContainer(B) ? "move" : (A.getX() + (A.getWidth() / 2) > Ext.lib.Event.getPageX(D) ? "movebefore" : "moveafter");
                return (C.drop == "movebefore" ? "icon-element-move-before" : (C.drop == "moveafter" ? "icon-element-move-after" : "icon-element-move"))
            } else {
                C.drop = this.isContainer(B) ? "append" : (A.getX() + (A.getWidth() / 2) > Ext.lib.Event.getPageX(D) ? "appendbefore" : "appendafter");
                return (C.drop == "appendbefore" ? "icon-element-add-before" : (C.drop == "appendafter" ? "icon-element-add-after" : "icon-element-add"))
            }
        }
        C.drop = null;
        return false
    },
    notifyDrop: function (D, C, B) {
        var A = this.getTarget(C);
        if (B.config && !B.processed && B.drop) {
            this.appendConfig(A, B.config, true, B.drop, B.source, this.require(B.require).cfg);
            B.processed = true
        }
        return true
    },
    setObjectValue: function (A, B, E, D, C) {
        if (B == "xtype" && this.jsonId) {
            if (!Ext.ComponentMgr.isTypeAvailable(E)) {
                D = {
                    value: E,
                    encode: true
                };
                E = "panel";
                this.fireEvent("error", "setObjectValue", new SyntaxError("xtype " + D.value + " does not exists"))
            } else {
                D = null
            }
        }
        return Ext.ux.guid.plugin.Designer.superclass.setObjectValue.call(this, A, B, E, D, C)
    },
    setPropertyGrid: function (A) {
        this.propertyGrid = A;
        this.propertyGrid.jsonScope = this.getScope();
        A.on("beforepropertychange", function (C, E, B, D) {
            this.markUndo()
        }, this);
        A.on("propertyvalue", function (F, B, E, C, D) {
            this.setObjectValue(F, B, E, E);
            return false
        }, this);
        A.on("propertychange", function (C, E, B, D) {
    ...
    toolbox: function (visible) {
        if (!this._toolbox) {
            ...
            var filterByFn = function (rec, id) {
                    ...
                    if (i) {
                        return eval("this.activeElement instanceof " + i)
                    }
                    return true
                }.createDelegate(this);
        ...
        if (visible || visible === true) {
            ...
            if (this.fireEvent("beforehide", this._toolbox)) {
                this._toolbox.hide()
            }
        }
    }
