...
register 'menu.admin.espacio' => {
  ...
  action   => 'action.bde.view_disk_report'
};
...
sub load : Local {
  ...
  my $p_query = $p->{query};
  my $p_hist  = $p->{hist};
  my @data = $c->model('Espacio')->get_data({sort  => $p_sort,
                                             dir   => $p_dir,
                                             query => $p_query,
                                             hist  => $p_hist});
...
sub load_path : Local {
  ...
  my @data = $c->model('Espacio')->get_data_path({project => $p_project,
                                                  sort    => $p_sort,
                                                  dir     => $p_dir});
  $c->stash->{json} = {data => \@data};
  $c->forward('View::JSON');
...
sub load_total : Local {
  my ($self, $c) = @_;
  $c->stash->{total_compress} =
    $c->model('Espacio')->get_total_compress_size();
