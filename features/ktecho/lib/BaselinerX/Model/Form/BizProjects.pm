...
sub get_grid_proyectos {
    ...
    # Returns the array of hashes as a reference...
    return \@data;
}
...
sub get_grid_recursos {
    ...
    my $sql = $ver == 12 ? $self->get_grid_recursos_r12()
    ...
    my @data = $ver == 12 ? $har_db->db->array_hash($sql, "\\$cam\\BIZTALK%\\_AC%")
...
sub delete_project {
    ...
    $params->{prj_registro_gac} = ( $params->{prj_registro_gac} == 1      ) ? 'Si'
                                : ( $params->{prj_registro_gac} eq 'true' ) ? 'Si'
                                :                                             'No'
                                ;
...
sub get_combo_proyecto {
    ...
    my $rs = Baseliner->model('Harvest::BdeBtsNet')
        ->search( { bts_env => $cam }, { search => ['BTS_USA_NET'] } );
    rs_hashref($rs);
    my @data = $rs->all;
...
sub add_project_net {
    ...
    Baseliner->model('Harvest::BdeBtsNet')->create(
        {   bts_env     => $cam,
            bts_usa_net => 'S'
        }
    );
