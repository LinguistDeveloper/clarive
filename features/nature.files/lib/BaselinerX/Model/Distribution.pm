...
sub job_item_something {
  ...
  my @ls = Baseliner->model('Distribution')->map_item_list_by($item_list, $attr);
  wantarray ? @ls : \@ls;
}
...
sub job_package_list {
  ...
  my $self = shift;
  my $job  = shift || $self->id;
  my @ls = $self->job_item_something($job, 'packagename');
  wantarray ? @ls : \@ls;
}
...
sub tar_friendly_elements { # Int Str Str -> HashRef
  ...
  my @elements;
  if ($type eq 'new') {
    @elements = $os =~ m/win/i  ? @{$self->new_win_elements($job)}
              : $os =~ m/unix/i ? @{$self->new_unix_elements($job)}
              :                   die 'wrong OS!';  
  }
  if ($type eq 'del') {
    @elements = $os =~ m/win/i  ? @{$self->del_win_elements($job)}
              : $os =~ m/unix/i ? @{$self->del_unix_elements($job)}
              :                   die 'wrong OS!';
  }
  my %kv = $self->get_maps($os);
  my %map;
  for my $key (keys %kv) {
...
sub unix_del_elements {
  my $self = shift;
  my $job  = shift || $self->id;
  $self->tar_friendly_elements($job, 'unix', 'del');
  return;
}
...
sub win_del_elements {
  my $self = shift;
  my $job  = shift || $self->id;
  $self->tar_friendly_elements($job, 'win', 'del');
  return;
}
