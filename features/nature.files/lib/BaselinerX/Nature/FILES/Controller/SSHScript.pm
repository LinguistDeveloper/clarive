...
register 'config.nature.filedist.ssh' => {
          metadata=> [
                      ...
                      { id=>'ssh_host', label=>'Conexion', type=>'text', nullable=>0, extjs =>{width=>250}  },
                      { id=>'xorder', label=>'Orden', type=>'combo', store =>order_index() },
                      { id=>'type', label=>'Tipo', type=>'combo', store =>[BaselinerX::Nature::FILES::SSHScript->TIPO_PRE,BaselinerX::Nature::FILES::SSHScript->TIPO_POST] },
                      { id=>'xtype', label=>'Aplicar a mapeos',  type=>'combo', store =>BaselinerX::Nature::J2EE::Service::Deploy->getAllTipoDistribuciones() },
...
sub filedist_ssh_json : Path('/filedist/ssh/json') {
    my ( $self, $c ) = @_;
    	  my $tipo = $c->request->parameters->{tipo};    			  
		  my ($ns,$bl) =  BaselinerX::Session::ConfigState->getConfigState($c,$tipo);    	      	      	    
    	  my $scripts = BaselinerX::Nature::FILES::SSHScript->new( $ns,$bl );		  
...
sub filedist_ssh : Path('/filedist/ssh') {
	...
	parseStashData($self,$c,BaselinerX::Nature::FILES::Service::Filedist->TIPO_FICHEROS);
...
sub parseStashData{
    ...
    $c->stash->{url_script_store} = '/filedist/ssh/json?ns='. $ns . '&bl=' . $bl . '&tipo=' . $tipo;
    $c->stash->{url_script_submit} = '/filedist/ssh/submit?tipo=' . $tipo;
...
sub filedist_ssh_submit : Path('/filedist/ssh/submit') {
          my ($self,$c)=@_;
          my $p = $c->req->params;
 		  my ($ns,$bl) =  BaselinerX::Session::ConfigState->getConfigState($c,$p->{tipo});         
    	  my $scripts = BaselinerX::Nature::FILES::SSHScript->new( $ns,$bl );
