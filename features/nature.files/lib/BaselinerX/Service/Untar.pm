...
sub main {
  ...
  for my $ref (@{$tar_list}) {
    ...
    if ($from =~ m/win/i) {
      my ($rc, $ret) = $balix->execute("cd /D $ref->{staging}\\N.TESTERIC & C:\\APS\\SCM\\Cliente\\tar -xvf elements.tar");
      $log->debug($ret);
      _throw("Error while using tar on Windows") if $rc;

      $log->debug("Init xcopy");
      my $cmd = "xcopy /E /F /R /Y /I $ref->{staging}\\N.TESTERIC\\* \\\\${host}${to}";
      $log->debug("exec: $cmd");
