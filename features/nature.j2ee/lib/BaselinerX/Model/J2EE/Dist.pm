...
sub webBuild {
  ...
  *Dist = shift();
  ...
  my ($Pase, $PaseDir, $TipoPase, $EnvironmentName, $Entorno, $Sufijo)
   = ($Dist{pase},    $Dist{pasedir}, $Dist{tipopase},
      $Dist{envname}, $Dist{entorno}, $Dist{sufijo});
  ...
  my %BUILDFILES;
  my (%EAR,     %JAR,      %WAR,      %JARLIST, %EARLIST,
      %WARLIST, %EARFILES, %JARFILES, %EARPRJMAP);
  ...
  # TODO: esto deberá estar en el formulario de Infraestructura
  ...
  ($STA{maq}, $STA{puerto}, $STA{home}, $STA{user})
   = $inf->get_staging_unix_active();
  ...
  $log->debug( "Aplicaciones publicas marcadas en el formulario: " 
              . join ', ', @apl_publicas_verobs);
  my $verPublicaJ2EE = "";
  ...
  for my $verPublicaJ2EE (@apl_publicas_verobs) {
    ...
    foreach $ESTADO (@EstadoVerPubJ2EE) {
      if ($ESTADO eq "Obsoleto") {
        ...
        $log->warn("La versión pública $verPublicaJ2EE ha sido borrada por los responsables de la aplicación, si su pase hace uso de esta versión fallará el pase");
  ...
  foreach my $pubv (@{$pubver}) {
    next if ($pubv =~ /^IAS/);  # Ignoro publicas IAS aquí (está más abajo).
  ...
  $log->debug("Proyectos java a ignorar: ",
              join ', ', keys %{$PARAMS{proyectos_ignorar} || {}});
  ...
  if ((keys %buildfilesSUBAPL) > 0) {
    ...
    if ($nivel eq 'EAR') {
      ...
      $log->info(  "Subaplicaciones afectadas por el pase: " 
                 . join ', ', sort keys %SUBAPL);
      ...
      foreach my $subapl (keys %SUBAPL) {
        ...
        $log->info(  "Fichero build.xml para la subaplicacion $subapl y los proyectos: " 
                   . (join ', ', @SUBAPL_PRJ), $buildxml->data);
        $log->info(  "Ficheros que se generarán en la construcción de $earprj: "
                   . join ', ',
                   $Workspace->genfiles());
      ...
      foreach my $war (@WARS) {
        if ($Dist{subapl_check}) {    # El nombre del project xxx_WEB no indica la subapl, hay que chequear los nombres de los ears (p.ej.CLF)
      ...
      foreach my $subapl (keys %SUBAPL) {
        ...
        $log->info(  "Fichero $buildfileBaseliner para la subaplicacion $subapl y los proyectos: " 
                   . (join ', ', ($SUBAPL{$subapl})), $buildxml->data);
      ...
      foreach my $ejb (@EJBS) {
        if ($Dist{subapl_check}) {    # El nombre del project xxx_EJB no indica la subapl, hay que chequear los nombres de los ears (p.ej.CLF)
...
sub nivelDist {
  ...
  *Dist = shift;
  ...
  @control_env{split /,/, $bde_conf->{control_ear}};

  if ($bde_conf->{control_ear} eq "1" || ($control_env{$Dist{entorno}})) {
...
sub webDist {
  ...
  *Dist = shift;    
  ...
  my ($Pase, $PaseDir, $Entorno, $TipoPase, $EnvironmentName, $Sufijo)
   = ($Dist{pase},     $Dist{pasedir}, $Dist{entorno},
      $Dist{tipopase}, $Dist{envname}, $Dist{sufijo});
  ...
  if (@{$Dist{prjlist}}) {
    ...
    foreach my $prj (@{$Dist{prjlist}}) {
      ...
      if ($subapl eq "") {    # No lo tengo! lo intento a mi manera
  ...
  if ($Dist{tar}) {
    foreach my $htptar (@{$Dist{tar}}) {
      ...
      elsif ("WAS" eq uc($destinoEstatico)) {    # va al WAS, separado del WAR (nivel eq 'TAR')
        if ($Dist{nivel} =~ /EAR|WAR/) {
          # no debería pasar por aquí, el staticExtensions no debería haber
  ...
  *Dist = shift;
  ...
  try {
    ...
    if (!$INSTAGING{"$buildhome"}) {
      ...
      # Descomprime
      ($RC, $RET) = $balix->executeas($STA{user},
                                      qq|cd $STA{buildhome} ; rm -f "${sta_tarfile}"; gzip -f -d "${sta_tarfile}.gz" ; $tarExecutable xvf "${sta_tarfile}"; rm -f "${sta_tarfile}"|);

      if (($RC ne 0) or ($RET =~ m/unexpected/i) or ($RET =~ m/no space/i)) {
        _throw "Error al descomprimir ${sta_tarfile}.gz (¿Falta espacio en disco en el servidor de staging $STA{maq}?): $RET";
      }
      else { 
    ...
    my $resolver = BaselinerX::Ktecho::Inf::Resolver->new({entorno => $Entorno,
                                                           sub_apl => $subapl,
                                                           cam     => $CAM});                                                                
    ...
    if ($RC eq 0) {
      ...
      $log->warn(
        "El path especificado para buscar el JDK no se encuentra en la máquina de Staging de WAS. Se compila usando el JDK por defecto en esa máquina."
      );
    }
    ...
    if ($PARAMS{PUB}) {
      ...
      $log->info("Transfiriendo librerías públicas generadas por la aplicación $EnvironmentName en staging. Espere...");
...
sub backupWEB {
  ...
  try {
    ...
    # CONFIG
    if (($DEST{tech} =~ /^IAS|^EDW4J|^J2EE_BDE/) && ($DEST{config_dir} ne "")) {    # es IAS
...
sub restoreWEB {
  ...
  $log->debug("Backups encontrados para '$EnvironmentName', '$Entorno', '$Sufijo', '$subapl'",
              _dump(%BACKUPS));
    ...
    if ($localfile && -e $localfile) {
      ...
      if (($tipo eq "EAR") && ($bde_conf->{was_script})) {
        ...
        try {
          ...
          if ($RC ne 0) {
            my $msg = "Restore $cnt: error durante la transmisión del fichero EAR.", $RET;
            $log->error($msg);
            die;
          }
          ...
          if ($RC ne 0) {
            $msg = "Restore $cnt: error al cambiar el permiso del EAR instalado $remfile ($DEST{was_user}:$DEST{was_group}:700).", $RET;
            $log->info($msg);
            die;
          }
          ...
          if (($RC ne 0) && ($RC ne 2) && ($RC ne 512)) {
            $msg = "Restore $cnt: error al ejecutar el script de instalación del EAR.", $RET;
            $log->info($msg);
            die;
          }
...
sub wasLogFiles {
  ...
  if (length($dest_logdir) > 1) {    # que no sea root /
    ...
    $log->warn("AVISO: Campo 'Directorio de logs' de la subaplicación $subapl en el formulario de infraestructura está vacío. Rellénelo para leer logs de trazas.");
  }
}
...
sub configDist {
  ...
  if ($locConfig && $localfile && $remfile && (-e $localfile)) {
    ...
    # BORRO /config/*  
    # TODO debería hacer un backup pequenin localmente
...
sub iasVersionPublico {
  ...
  $log->warn("IAS: no he localizado la versión de IAS en ias.jar. Uso de librerías públicas de IAS descartado") if (!$iasversion);
  return "";
  # ¿chequea si la release está disponible para este entorno?
}
...
sub staticExtensions {
  ...
  my ($cam, $CAM) = get_cam_uc($EnvironmentName);
  my $esEstatico = $inf->get_inf({sub_apl => $subapl},
                                 [{column_name => "WAS_STATIC_CONTENT"}]);
  ...
  if ("WEB" eq uc($esEstatico)) {
    ...
    if ($nivel =~ /EAR|WAR/) {    # no hay exclusiones, ya van dentro del fichero EAR/WAR al Websphere
      return ();
    }
    else {
...
sub htpDist {
  ...
  if ((-e $localfile) && $tarfile) {
    ...
    # true si hay ficheros para hacer backup, si no el tar estará vacío y da
    ...
    $log->info("Contenido estático: Fin del envío de contenido estático."); 
  }
  else {
  ...
  *Dist = shift();
  ...
  if ($subapl) {
    ...
    if (($DEST{tech} =~ /^IAS|^EDW4J|^J2EE_BDE/) && ($DEST{config_dir} ne "")) {
      ...
      if ($PRJMAP{$prj}{WARS}) {
        ...
        foreach my $wardir (@WARPRJS) {
          if ($wardir) {
            # TODO: el nombre del VP de config podría cambiar.
    ...
    if ($Dist{nivel} ne 'CONF') {
      ...
      open my $fh, '>', '/home/apst/scm/temp/ret';    # XXX
      print {$fh} $RET;                               # XXX
      ...
      if (($RC != 0) && ($RC != 2) && ($RC != 512)) {
        ...
        my $msg = "Fichero '$DEST{home}/$file' desplegado " . ($RC eq 2 ? "con warnings " : "") . "(RC=$RC).", $RET;
        if (($RC == 2) || ($RC == 512)) { 
          $log->warn($msg, $RET);
        }
        else { 
    ...
    if ($DEST{reinicio} eq 1) {
      ...
      if ($RC != 0) {
        if ($RC == 512) {    # es que estaba caido, no problem
        ...
        my $msg = "Ok. WAS $dest_clone parado " . ($RC eq 2 ? "con warnings " : "") . " con usuario '$DEST{was_user}' (RC=$RC).", $RET;
        if ($RC eq 2) {
          $log->warn($msg, $RET); 
        }
        else { 
...
sub accionWAS {
  ...
  # TODO Aqui lo suyo sería dar un error, pero por el momento, prudencia...
  return 'udpateEAR';
}
...
sub earObligatorio {
  ...
  return ($bde_conf->{ear_obligatorio} =~ m{$Entorno}i);
}
...
sub getJDKVersion {
  ...
  my $jdkVersion = $inf->get_inf({sub_apl => $subapl},
