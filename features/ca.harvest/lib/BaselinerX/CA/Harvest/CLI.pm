...
## General attributes
has 'tempdir' => ( is=>'rw', isa=>'Any', default=>$ENV{BASELINER_TEMP} || $ENV{TEMP} );
has 'harvesthome' => ( is=>'rw', isa=>'Any', default=> $ENV{SCMHOME} || $ENV{HARVESTHOME} );
## Context attributes
has 'broker' => ( is=>'rw', isa=>'Any', default=>'localhost' );
has 'login' => ( is=>'rw', isa=>'Any', default=>find_dfo()  );
has 'project' => ( is=>'rw', isa=>'Any' );
...
sub _logfile {
    my ($self,$prefix)=@_;
	my $logpath = File::Spec->catfile( $self->tempdir, 'harvest-cli' );
	_mkpath( $logpath );
	my $logfile = File::Spec->catfile( $logpath, "$prefix$$-"._nowstamp.".log");
    Path::Class::Dir->new( $logfile );
}

sub _quote_comma { '"'.join('","', @{ $_[0] || [] }) . '"'; }
...
sub run {
    ...
    for( grep /^\-/, keys %p ) {
        if( defined $p{$_} ) {
             if ($_ eq -F) {
				 $args = qq{ "$p{ $_ }" } . $args ;  ## ie. -F fichero para hci"
			 } else {
				 if( ref($p{$_}) eq 'ARRAY' ) {
    ...
    if( ref $p{args} eq 'ARRAY' ) {
        $args.= " ". _quote_space( $p{args} );
    } else {
        $args.= $p{args} if $p{args};
    }
	my $arg_data = qq{-o "$logfile" -b "$self->{broker}" $self->{login} $args }; #"
    my $farg = $self->write_argfile($arg_data);
	_log "Fichero argumentos $farg:".$arg_data;
	_log "Comando:".qq{$p{cmd} -i "$farg 2>&1"};
...
sub select_versions {
    my ($self, %p)=@_;
    my $mask = $p{mask} || '*';
    my $r = $self->run( cmd=>'hsv', -en=>$p{project}, -st=>$p{state}, -vp=>$p{vp}, -p=>$p{package}, -s=>$mask, -ib=>'tb' );
	_log "hsv rc: ".$r->{rc};
    my @versions;
    unless( $r->{rc} ) {
        my @data = split /\n/, $r->{msg}; 
